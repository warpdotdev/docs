---
name: style_lint
description: Scan Warp GitBook documentation for style guide violations including formatting issues (Settings path format, UI element format, header case, missing frontmatter, image alt text, callout syntax) and terminology issues (product name casing, Oz terms to avoid, deprecated terms). Run with --changed for PR workflows or --all for periodic audits. Optionally auto-fix high-confidence issues with --fix.
---

# Style Lint

Scan Warp GitBook documentation for formatting and terminology issues defined in the style guide (`AGENTS.md`).
...
- `--slack-notify`: Send results to `#growth-docs` Slack channel (only sends when issues are found)
...
- **Callout syntax**: Malformed `{% hint %}` tags (missing `{% endhint %}`, typos in style names)
