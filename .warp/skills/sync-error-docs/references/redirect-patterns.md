...
2. A **GitBook space redirect** from the underscore filename to the hyphen filename (within the reference space)

## 1. Site-level redirect (GitBook API)

This redirect is created via the GitBook API using `scripts/gitbook_redirects.py`. It requires the `GITBOOK_TOKEN` environment variable.
...
```bash
python3 gitbook/scripts/gitbook_redirects.py list --search "/errors/"
```
...
## 2. GitBook space redirect (.gitbook.yaml)

This redirect lives in `gitbook/docs/reference/.gitbook.yaml` and handles in-space navigation where someone might visit the underscore form of the path.
