...
- `{underscore\_code}` — Same but with underscores escaped for GitBook H1 headings (e.g., `insufficient\_credits`)
...
- The H1 title uses the underscore form with `\_` escaping (GitBook renders underscores as italics otherwise)
...
- Platform errors should include a `{% hint style="info" %}` callout explaining the ERROR vs FAILED distinction (see `authentication-required.md` for an example)
