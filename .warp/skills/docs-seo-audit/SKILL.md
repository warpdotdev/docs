...
- `--repo-root PATH` — Path to the gitbook repo root. Enables mapping live URLs to local source files in the report.
...
- `og_title_mismatch` — OG title differs from the title tag (usually auto-matched by GitBook).
...
- Duplicate title "Overview | Agents | Warp" on 4 pages:
  - /agent-platform (source: docs/agent-platform/README.md)
  - /agent-platform/local-agents/overview (source: docs/agent-platform/local-agents/overview.md)
  ...
...
1. `references/gitbook-seo.md` in this skill directory — explains the non-obvious way GitBook generates title tags from SUMMARY.md link text (not the H1 heading).
2. `AGENTS.md` at the gitbook repo root — the single source of truth for documentation style, voice, terminology, and formatting. All titles and descriptions you write must follow these conventions (terminology, sentence case, active voice, frontmatter description format).

### Key principles

1. **Title tags come from SUMMARY.md link text**, not the H1 heading. To fix a title, change the link text in the relevant space's `SUMMARY.md`.
