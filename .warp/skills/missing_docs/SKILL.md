...
name: missing_docs
description: >-
  Find and fill documentation gaps in Warp's GitBook docs by auditing coverage
...
2. **CLI command coverage** — compares `warp_cli/src/lib.rs` subcommands against `docs/reference/cli/`
3. **API endpoint coverage** — compares `router/router.go` routes against `docs/reference/api-and-sdk/` and the OpenAPI spec
...
To find warp-internal and warp-server, search for sibling directories of the gitbook repo. If not found, ask the user.
...
- `AGENTS.md` (gitbook repo root) — full documentation style guide
