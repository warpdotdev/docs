#!/usr/bin/env python3
"""
Missing Docs Audit Script for Warp GitBook Documentation
...
SKIP_DIRECTORIES = {"_book", "node_modules", ".git", ".gitbook"}
...
def find_repo(name: str, explicit_path: str | None, gitbook_root: Path) -> Path | None:
    ...
    # Try sibling directory (gitbook is at .../code/gitbook, look for .../code/<name>)
    sibling = gitbook_root.parent / name
    if sibling.exists():
        return sibling
    return None
...
def read_all_docs_text(docs_root: Path) -> dict[str, str]:
    ...
    for f in find_markdown_files(docs_root):
        try:
            rel = str(f.relative_to(docs_root.parent))  # relative to gitbook root
...
def main():
    ...
    # Find repos
    gitbook_root = SKILL_DIR.parent.parent.parent  # .warp/skills/missing_docs -> gitbook root
    docs_root = gitbook_root / "docs"

    if not docs_root.exists():
        print(f"Error: docs directory not found at {docs_root}", file=sys.stderr)
        sys.exit(1)

    warp_internal = find_repo("warp-internal", args.warp_internal, gitbook_root)
    warp_server = find_repo("warp-server", args.warp_server, gitbook_root)
