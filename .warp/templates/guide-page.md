...
{% embed url="[YouTube or Loom URL — remove this block if no video]" %}
...
[LINK VERIFICATION: Before publishing, verify every internal link points to an existing page by checking the relevant SUMMARY.md. If a target page is planned but not live, use the closest existing page and add a TODO comment.]
