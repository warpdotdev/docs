...
{% hint style="info" %}
This page covers the [managed architecture](README.md#managed-architecture) with the Docker backend. For the Kubernetes backend, see [Managed: Kubernetes](managed-kubernetes.md). For host execution without a container runtime, see [Managed: Direct](managed-direct.md). If you'd rather invoke agents yourself, see [Unmanaged](unmanaged.md).
{% endhint %}
...
The worker can be configured entirely via CLI flags, or via a YAML [config file](reference.md#config-file) for more complex setups.
...
The worker can take configuration either via CLI flags or via a YAML [config file](reference.md#config-file). CLI flags take precedence over config file values.
...
Pass it with `--config-file config.yaml`. See the [self-hosted worker reference](reference.md) for the full flag and config schema.
...
The worker automatically uses credentials from your Docker config (`~/.docker/config.json`) when pulling task images. If your [environments](../environments.md) use images from a private registry, authenticate the worker's host first:
...
Once your Docker worker is connected, route tasks to it with `--host "<your-worker-id>"`. Routing is the same across all managed backends — see [Routing runs to self-hosted workers](README.md#routing-runs-to-self-hosted-workers) for CLI, scheduled, integration, API, and web UI examples.

***

## Related pages

* [Self-hosting quickstart](quickstart.md) — ~10-minute path to a running Docker worker.
* [Self-hosted worker reference](reference.md) — Full CLI flag and config file schema.
* [Environments](../environments.md) — Define the Docker image, repos, and setup commands for tasks.
* [Security and networking](security-and-networking.md) — Data boundaries, egress, and Docker socket considerations.
* [Troubleshooting](troubleshooting.md) — Common issues with the Docker backend.
