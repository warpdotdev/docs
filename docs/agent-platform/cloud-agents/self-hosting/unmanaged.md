...
{% hint style="info" %}
Unmanaged is the right choice if you already have a system that schedules work (CI, internal orchestrators, cron, dev environments). If you'd rather have Oz trigger and route runs from Slack, Linear, schedules, or the API, use the [managed architecture](README.md#managed-architecture) instead.
{% endhint %}
...
* **The Oz CLI** installed on the machine where agents will run. See [Installing the CLI](https://docs.warp.dev/reference/cli#installing-the-cli) for platform-specific instructions.
* **A Warp API key** — For automation, create a team-scoped API key in the Warp app at **Settings** > **Cloud platform** > **Oz Cloud API Keys**. See [API Keys](https://docs.warp.dev/reference/cli/api-keys) for details.
...
* **Attach to** via [Agent Session Sharing](../../warp-agents/session-sharing.md) to monitor or steer.
* **Query** through the [Oz API/SDK](https://docs.warp.dev/reference/api-and-sdk/agent) for custom dashboards or monitoring.

Unmanaged sessions benefit from the same shared configuration as other cloud agent runs — [MCP servers](../mcp.md), [secrets](../secrets.md), Warp Drive context, and saved prompts all apply.

***

## Related pages

* [Self-hosting overview](README.md) — Compare managed and unmanaged, plus the architecture decision guide.
* [GitHub Actions integration](../integrations/github-actions.md) — Run agents in CI with the official action.
* [Deployment patterns](../deployment-patterns.md) — Pattern 1 (CLI-only) explains the unmanaged model conceptually.
* [Oz CLI](https://docs.warp.dev/reference/cli) — Full CLI reference for `oz agent run` and related commands.
* [Agent Session Sharing](../../warp-agents/session-sharing.md) — Attach to running sessions to monitor or steer them.
