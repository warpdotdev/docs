...
Reference for the `oz-agent-worker` daemon: CLI flags and the full YAML config-file schema for all three [managed backends](README.md#managed-architecture) — Docker, Kubernetes, and Direct.

{% hint style="info" %}
This page documents every flag and config option. For installation and backend-specific setup walkthroughs, see [Managed: Docker](managed-docker.md), [Managed: Kubernetes](managed-kubernetes.md), or [Managed: Direct](managed-direct.md). This reference applies to the managed architecture only; the [unmanaged architecture](unmanaged.md) uses `oz agent run` instead.
{% endhint %}
...
* `--backend` — Backend type: `docker` (default), `kubernetes`, or `direct`. See [Managed: Kubernetes](managed-kubernetes.md) and [Managed: Direct](managed-direct.md) for backend-specific setup.
...
* [Managed: Docker](managed-docker.md) — Docker backend setup, connectivity, and private registries.
* [Managed: Kubernetes](managed-kubernetes.md) — Kubernetes backend setup, Helm chart, pod template, and operational notes.
