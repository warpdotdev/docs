...
* **The Oz CLI** (for routing a test run) — See [Installing the CLI](https://docs.warp.dev/reference/cli#installing-the-cli).
...
Open the [Oz dashboard](https://oz.warp.dev), find the new task, and confirm the session transcript shows the agent running against your worker. You can attach to the session at any time via [Agent Session Sharing](../../warp-agents/session-sharing.md) to monitor or steer it.

***

## Next steps

* [Unmanaged quickstart](unmanaged.md#unmanaged-quickstart) — ~5-minute CLI-only path: run `oz agent run` in your CI, Kubernetes pod, or dev box with no worker daemon and no Docker requirement.
* [Managed: Docker](managed-docker.md) — Full Docker backend setup, including private registries, volume mounts, and runtime configuration.
* [Environments](../environments.md) — Define a repository, Docker image, and setup commands so agents have a reproducible workspace for every run.
* [Routing runs to self-hosted workers](README.md#routing-runs-to-self-hosted-workers) — How to route tasks from schedules, integrations (Slack, Linear), the API, and the Oz web app.
* [Managed: Kubernetes](managed-kubernetes.md) — Deploy workers into a Kubernetes cluster with Helm.
* [Self-hosted worker reference](reference.md) — All CLI flags and config file options.
