...
{% hint style="info" %}
This page covers the [managed architecture](README.md#managed-architecture) with the Direct backend. For container-based task isolation, see [Managed: Docker](managed-docker.md) or [Managed: Kubernetes](managed-kubernetes.md). For invocation-driven use cases, see [Unmanaged](unmanaged.md).
{% endhint %}
...
Or with a [config file](reference.md#config-file):
...
* [Self-hosted worker reference](reference.md#direct-backend-config) — Full config schema for the Direct backend.
...
* [Security and networking](security-and-networking.md) — Data boundaries and security considerations for the Direct backend.
* [Troubleshooting](troubleshooting.md#direct-backend) — Common Direct-backend issues.
