...
The [Helm chart](managed-kubernetes.md) includes built-in support for metrics. Enable metrics with `metrics.enabled=true`:
...
* [Self-hosted worker reference](reference.md) — CLI flags, config file schema, and metrics environment variables.
* [Managed: Kubernetes](managed-kubernetes.md) — Helm chart deployment, including metrics values.
* [Troubleshooting](troubleshooting.md) — Diagnostics for metrics issues and other common problems.
* [Security and networking](security-and-networking.md) — Network egress and data boundaries.
