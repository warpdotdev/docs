---
description: >-
  Security model, data boundaries, and network requirements for self-hosted
  Oz cloud agents — including per-backend considerations and BYOLLM.
---
...
{% hint style="info" %}
This page applies to both the [managed](README.md#managed-architecture) and [unmanaged](unmanaged.md) architectures. Backend-specific notes call out Docker-, Kubernetes-, and Direct-only considerations.
{% endhint %}
...
**Routes through Warp's backend** (under [Zero Data Retention (ZDR)](https://docs.warp.dev/enterprise/security-and-compliance/security-overview#zero-data-retention-zdr)):
...
**GitHub (`github.com`)** — only with the managed architecture, when using a Warp [environment](../environments.md) with configured GitHub repositories.
...
* [Self-hosting overview](README.md) — Managed vs unmanaged and architecture decision guide.
* [Security overview](https://docs.warp.dev/enterprise/security-and-compliance/security-overview) — Warp's broader security model, including ZDR.
* [Bring Your Own LLM (BYOLLM)](https://docs.warp.dev/enterprise/enterprise-features/bring-your-own-llm) — Route inference through your own cloud provider accounts.
* [Self-hosted worker reference](reference.md) — CLI flags and config schema, including every security-relevant option.
