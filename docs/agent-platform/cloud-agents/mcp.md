...
* **At run time** — pass `--mcp` when calling `oz agent run` or `oz agent run-cloud`. See [MCP Servers](https://docs.warp.dev/reference/cli/mcp-servers) in the CLI reference for the full syntax.
