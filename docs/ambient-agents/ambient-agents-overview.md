...
* **Execution context (optional)**: an [Environment](../platform/cli/integrations-and-environments.md#what-is-an-environment) that defines the repo, image, and startup commands the agent should run with.
...
Ambient Agents run on the [Warp Platform](../platform/warp-platform.md), which provides the primitives for triggering work, orchestrating tasks, executing agents (optionally in Environments), injecting secrets, and inspecting results. You can adopt them incrementally, but most production deployments follow the same structure:

* Something **triggers** an agent task.
* The **orchestrator creates** and tracks the task.
* The agent **executes** on a host, optionally inside an [environment](../platform/cli/integrations-and-environments.md#what-is-an-environment), with whatever [secrets](agent-secrets.md) and credentials it needs.
...
* [Agent Session Sharing](../knowledge-and-collaboration/session-sharing/ambient-agents-session-sharing.md) lets authorized teammates attach to a running task to monitor progress and, where supported, steer the agent while it runs.
...
The Warp Platform exposes task visibility via the [**Agent API and SDKs**](../platform/agent-api-and-sdk/), so teams can…
...
* [Warp CLI](../platform/cli/)
* Web surfaces (where available)
* [Agent Session Sharing](../knowledge-and-collaboration/session-sharing/agent-session-sharing.md)
* [Agent Management UX](../agents/using-agents/managing-agents.md)
* Admin settings and [APIs](../platform/agent-api-and-sdk/)
...
* **Credit requirements**
  * Your team must have at least 20 [add-on credits](../support-and-billing/plans-and-pricing/add-on-credits.md) available to run an integration.
...
For more details, please refer to: [team-access-billing-and-identity-permissions.md](../platform/team-access-billing-and-identity-permissions.md "mention")
...
* [warp-platform.md](../platform/warp-platform.md "mention") — CLI, Agent API/SDK, orchestration, tasks, environments, hosts, integrations, and more.
* [cli](../platform/cli/ "mention") — shows how to run Warp’s agent in non-interactive mode from CI, scripts, or remote machines, including auth and common commands.
* [integrations-and-environments.md](../platform/cli/integrations-and-environments.md "mention") — explains how triggers (Slack/Linear/webhooks/schedules) create agent tasks and how environments provide the runtime context (repo, image, startup commands).
* [agent-api-and-sdk](../platform/agent-api-and-sdk/ "mention") — documents the REST API for creating, querying, and monitoring agent tasks programmatically.
* [agent-secrets.md](agent-secrets.md "mention") — covers how to store, scope, and inject credentials into agent runs safely.
* [mcp-servers-for-agents.md](mcp-servers-for-agents.md "mention") — describes how to configure MCP servers for agent tool access and how MCP configuration is applied across runs.
* [deployment-patterns.md](../platform/deployment-patterns.md "mention") (beta) — compares common ways to deploy Ambient Agents and when to use each.
* [team-access-billing-and-identity-permissions.md](../platform/team-access-billing-and-identity-permissions.md "mention") — explains team-level requirements, credit billing behavior, and the permission model for who can run, view, and steer Ambient Agent tasks.
