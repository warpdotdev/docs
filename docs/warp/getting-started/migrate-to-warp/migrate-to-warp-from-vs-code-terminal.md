...
The fastest way to bring over your VS Code terminal setup is to ask Warp's agent to translate `settings.json` directly. Warp ships a [`settings.toml` file](../../terminal/settings/README.md) and a bundled `modify-settings` skill that lets the agent read your existing config and write equivalent values into Warp's settings.
...
VS Code's terminal uses the color scheme from your overall editor theme. In Warp, pick a comparable theme from **Settings** > **Appearance** > **Themes**, or [create a custom theme](../../terminal/appearance/custom-themes.md) that matches your VS Code theme's `terminal.*` color tokens.

### Keybindings

Warp's [default keyboard shortcuts](../keyboard-shortcuts.md) are largely consistent with VS Code terminal shortcuts (splits, new tab, find). For any custom bindings you configured in VS Code, add them in Warp's **Settings** > **Keyboard shortcuts**.
...
VS Code's integrated terminal still works; use it for quick one-off commands, and jump to Warp when you need [blocks](../../terminal/blocks/README.md), [Agent Mode](https://docs.warp.dev/agent-platform/warp-agents), or [persistent sessions](../../terminal/sessions/session-restoration.md).

### Replace VS Code with Warp

Warp includes a built-in [code editor](../../code/code-editor/README.md) with Language Server Protocol (LSP) support, a [file tree](../../code/code-editor/file-tree.md), [find and replace](../../code/code-editor/find-and-replace.md), and [Vim keybindings](../../code/code-editor/code-editor-vim-keybindings.md). Combined with [Code Review](../../code/code-review.md), many developers use Warp as their primary editor and drop VS Code entirely.
...
| --- | --- |
| Split terminal | [Split panes](../../terminal/windows/split-panes.md) |
| Multiple terminals (tab strip) | [Tabs](../../terminal/windows/tabs.md), [vertical tabs](../../terminal/windows/vertical-tabs.md) |
| Tasks (`tasks.json`) | [YAML workflows](../../terminal/entry/yaml-workflows.md) or [tab configs](../../terminal/windows/tab-configs.md) |
| Terminal profiles | [Tab configs](../../terminal/windows/tab-configs.md) + per-session shell overrides |
| Shell integration | Built in via [Warpify](../../terminal/warpify/README.md); enables working-directory tracking and command-level [blocks](../../terminal/blocks/README.md) |

For an overview of what Warp adds beyond a terminal, see [Coding in Warp](../coding-in-warp.md).
