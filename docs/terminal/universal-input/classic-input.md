...
* [prompt.md](../appearance/prompt.md "mention") — Use a fully customizable Warp prompt or your shell prompt, with support for PS1 and same-line prompts.
* [input-position.md](../appearance/input-position.md "mention") — Choose where the input appears in Warp, including both the prompt and the command line.
* [editor](../editor/ "mention")— Warp’s input editor works like a modern IDE, with rich editing capabilities not found in most terminals.
* [entry](../entry/ "mention") — Access Warp’s features for command history, synchronized inputs, YAML workflows, and more.
* [text-selection.md](../more-features/text-selection.md "mention") — Use smart selection or rectangular (column) selection to highlight text precisely without tedious cleanup.
...
<figure><img src="/broken/files/m10AINGRfWaTyv9jdRPT" alt="Agent Mode makes a suggestion to run a command."><figcaption><p>Agent Mode makes a suggestion to run a command.</p></figcaption></figure>
...
Warp lets you choose from a curated list of LLMs for use in Agent Mode. By default, Warp uses **Claude 4 Sonnet** for auto, but you can switch to other supported models. For all available models, please refer to [model-choice.md](../../agents/using-agents/model-choice.md "mention").
