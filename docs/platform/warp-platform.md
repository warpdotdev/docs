...
The [cli](cli/ "mention") is the **headless interface** for running Warp agents in non-interactive mode. It’s commonly used in CI, scripts, and server environments where there is no interactive UI. For interactive workflows, use the [agent](../agents/agents-overview.md) embedded in Warp’s desktop app.
...
* Exposes task lifecycle operations via the [CLI](cli/) and a [REST API](agent-api-and-sdk/) (create tasks, query history, and inspect status/outputs).
...
[Environments](cli/integrations-and-environments.md#what-is-an-environment) define the execution context an agent should run in.
...
The Warp [Agent API](agent-api-and-sdk/#agent-api) is the HTTP interface to the Warp Platform. It lets you create and inspect Ambient Agent tasks from any system (CI, cron, backend services, internal tools), without requiring the Warp desktop app.
...
If you’re building an integration (CI, Slack bots, internal tooling, orchestrators), the [SDKs](agent-api-and-sdk/#agent-sdk) are typically the quickest and safest starting point.
...
* [Management UI](../agents/using-agents/managing-agents.md): lists tasks, status, timing, metadata, and history.
* [Agent Session Sharing](../knowledge-and-collaboration/session-sharing/agent-session-sharing.md): authorized teammates can attach to a running task to monitor and, where supported, steer it.
* [API](agent-api-and-sdk/)s and SDKs: query task history, build monitoring, and generate reports.
...
* [rules / guardrails](../knowledge-and-collaboration/rules.md)
* [saved prompts](../knowledge-and-collaboration/warp-drive/prompts.md)
