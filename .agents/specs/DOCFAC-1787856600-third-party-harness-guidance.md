...
9. This is a pure data/copy change, so a regression test is testing-exempt: a string-presence test would be tautological and brittle. The repository's documented gates still pass: run `trunk fmt`, `trunk check`, and `npm run build` from the docs root.
10. Exercise the built docs site with computer use. Record a short video that opens the Factory agents page, scrolls through the third-party harness setup, shows the paid-plan/pricing guidance and inline auth caveats, and follows at least one internal harness/authentication link. Confirm the rendered MDX has no layout, callout, code-block, or link regressions, attach the recording to the task and PR, and do not commit media.
