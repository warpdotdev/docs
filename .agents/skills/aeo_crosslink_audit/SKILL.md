...
- **Peec snapshot** - Read the snapshot file at `/workspace/buzz/aeo-snapshots/docs/agents-orchestration/latest.json`. This file is generated by a separate workflow and contains recent Peec prompts, query clusters, recommendations, source URLs, candidate docs areas, and open questions for the agents/cloud-agents/orchestration topic area. Use the `prompts`, `query_clusters`, `recommendations`, and `candidate_docs_areas` fields to identify user language and link opportunities. If the snapshot file does not exist or is older than 30 days (check `generated_at`), fall back to Peec MCP if available, or note that Peec data could not be loaded.
...
1. **Gather source signals.** Read the Peec snapshot file and Google Search Console data, when available, to identify relevant user language, prompts, recommendations, or pages.
