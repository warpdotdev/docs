...
Agent Mode is a mode in Warp that lets you perform any terminal task with natural language. Type the task into your terminal input, click \<enter>, and Warp AI runs highly accurate commands tailored to your environment.\
...
* Use keyboard shortcuts to toggle into Agent Mode `Cmd-I` (Mac) or `Ctrl-I` (Linux)  or type `*<space>`
* Click the “AI” sparkles icon in the menu bar, and this will open a new terminal pane that starts off in Agent Mode
...
When you are in Agent Mode, a sparkles icon will display inline with your terminal input:

<figure><img src="../../.gitbook/assets/undo_my_git_commit.png" alt=""><figcaption><p>The sparkles on the command line indicate Agent Mode is active.</p></figcaption></figure>



### Auto-detection for natural language and configurable settings

The feature Warp uses to automatically detect natural language is completely local. None of your input is sent to AI unless you click \<enter> in Agent Mode.&#x20;
...
<figure><img src="../../.gitbook/assets/banner_for_auto-detection_first_experience.png" alt=""><figcaption><p>Warp displays an option to toggle natural language detection on / off</p></figcaption></figure>

### How to exit Agent Mode

You can quit Agent Mode at any point with `Esc` to escape Agent Mode or `Cmd I` / `Ctrl I` to toggle out of Agent Mode.&#x20;

### How to run commands in Agent Mode

Once you have typed your question or task in the input, click \<enter> to execute your AI query. Agent Mode will send your request to Warp AI and begin streaming output in the form of an AI block.
...
If Agent Mode finds a suitable command that will accomplish your task, it will describe the command in the AI block. It will also fill your terminal input with the suggested command so you can hit enter to run the command.
...
<figure><img src="../../.gitbook/assets/Screenshot 2024-06-14 at 5.07.37 PM.png" alt=""><figcaption><p>Agent Mode makes a suggestion to run a command.</p></figcaption></figure>
...
You must explicitly agree and hit \<enter> to run the requested command. When you hit enter, both the command input and the output will be sent to Warp AI.

If you do not wish to send the command or its output to AI, you can hit cancel or `^ C` to exit Agent Mode and return to the traditional command line. No input or output is ever sent to Warp AI without your explicit action.&#x20;

<figure><img src="../../.gitbook/assets/Screenshot 2024-06-14 at 5.13.02 PM.png" alt=""><figcaption><p>Warp AI asks permission to run a command and read the output.</p></figcaption></figure>
...
{% hint style="info" %}
Long conversations can have high latency. We recommend creating a new conversation when possible for distinct tasks or questions where previous context isn't relevant.
{% endhint %}
...
If you're already in Agent Mode, there are additional ways to add context to. your query:

1. You can use `Cmd` + arrow key on Mac / `Ctrl` + `Shift` + arrow key on Linux  to attach the previous block as context to the query. You can then use your arrow keys to pick another block.
2. You may also use your mouse to select blocks in your session. Hold `Cmd` on Mac / `Ctrl-Shift` on Linux  as you click on other blocks in order to extend your block selection.

To clear attached blocks from a query, you can use `Cmd` and arrow down until the blocks are removed. You can also click on any attached block in the blocklist to detach it.
...
To enter follow-up mode manually, use `Cmd-Y` (Mac) / `Ctrl-Y` (Linux).&#x20;
...
To start a new conversation manually, use `Cmd-Y` (Mac) / `Ctrl-Y` (Linux), or backspace.

**Note on context truncation**
...
Every Warp plan includes a set number of Warp AI requests per user per month. Please refer to [warp.dev/pricing](https://www.warp.dev/pricing) to compare plans.\
...
* You have to explicitly grant approval for any command the AI wants to run.
...
* All actions are performed through terminal commands. All commands are inspectable. You may click on requested commands to see their output. All suggested commands are run as regular Warp blocks you can read.&#x20;
* [Suggested commands](agent-mode.md#agent-mode-command-suggestions) are not read by Warp AI.
* While [requested commands](agent-mode.md#agent-mode-requested-commands) and their output are sent to Warp AI, you get to cancel out of a requested command at any time.
* You can inspect all data that’s leaving your machine using [Network Log](https://docs.warp.dev/features/network-log).
...
* Agent Mode works better with Warp's default prompt settings, where the prompt starts on a new line, than it does with a same line prompt; if you are using same line prompt, the cursor will jump from the end of the single line ot the start of the input box when you switch to Agent Mode.
