...
Every conversation includes the session's current working directory as context. The CLI starts in the directory where you ran `warp`, and when you `cd` inside the session, project rules and skills re-scope to the new directory automatically.
...
2. Use `↑` and `↓` to select a skill, then press `Enter`. The CLI inserts `/skill-name ` into the input so you can add extra instructions before running it.
