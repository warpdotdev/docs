...
* **[Shell commands](/cli/shell-commands/)** - Commands run in your session, with output streamed into the transcript.
...
* **Multi-file edits** - Nest each file's section, indented, under one collapsible summary header (for example, `Edited 3 files`).
