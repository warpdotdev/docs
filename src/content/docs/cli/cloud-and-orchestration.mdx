...
To keep working on a run from your terminal instead, pull it into the CLI. Finished cloud runs appear in the conversation history alongside local conversations:
...
You can also open a conversation directly at startup with `warp --resume <token>`; [managing conversations in the CLI](/cli/agent-conversations/#managing-conversations) covers conversation history and resume details.
...
To start an orchestration, ask the agent to parallelize the work, for example "split this migration across three agents". Your prompt can also shape the configuration: ask for a specific model to manage token spend - for example, keep the parent on a premium model while child agents handle research on a cheaper open-source model. When the agent decides to spawn child agents, the CLI shows an approval card titled **Can I start additional agents for this task?** that summarizes the agents it wants to launch and the run-wide configuration.
