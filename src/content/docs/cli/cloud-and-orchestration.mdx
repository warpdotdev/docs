...
To see the work a cloud agent did, start in the browser: open the run in the {VARS.WEB_APP} to review its transcript and changes, steer it, or continue the conversation there. [Viewing cloud agent runs](/platform/viewing-cloud-agent-runs/) covers that flow.

If you then want to keep working on a run from your terminal, you can pull it into the CLI. Finished cloud agent runs appear in the CLI's conversation history alongside local conversations:
...
* **The run must be finished** - Cloud runs that are still in progress or blocked can't be opened in the CLI. Steer those from the browser instead.
...
To start an orchestration, ask the agent to parallelize the work, for example "split this migration across three agents". Your prompt can also shape the configuration: ask for a specific model to manage token spend - for example, keep the parent on a premium model while child agents research on a cheaper open-source model. When the agent decides to spawn child agents, the CLI shows an approval card titled **Can I start additional agents for this task?** that summarizes the agents it wants to launch and the run-wide configuration, including the model.

By default, no child agents launch until you approve. The **Run orchestrated agents** permission in your profile controls this: **Always allow** launches child agents without asking, and **Never** blocks them entirely. See [Permissions and profiles](/cli/permissions-and-profiles/).
...
* **Cloud child agents** - Each cloud child runs in a cloud [environment](/platform/environments/). Unlike local children, cloud children can run a different harness, such as Claude Code or Codex, chosen on the approval card's harness page. A cloud child's session shows the run's live status and a link; press `Enter` or click the link to open the run in the {VARS.WEB_APP}. If a cloud child can't start because GitHub authentication is required, its session shows the authentication link; authenticate, then ask the agent to run the orchestration again.
...
A focused local child behaves like a regular conversation: send it follow-up prompts and respond to its permission requests. If a child's tab shows a blocked status, switch to that child to respond to its pending request.

### Kill a child agent

`Ctrl+C` kills a child agent: it stops the child's work - cancelling the run in the cloud for a cloud child - and removes the child's tab and conversation, returning you to the orchestrator. There are two ways to do it:
...
`Ctrl+C` keeps its usual behavior in the orchestrator's own session; the kill paths apply only while a child is selected or focused.
