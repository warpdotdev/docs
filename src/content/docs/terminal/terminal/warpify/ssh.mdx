...
:::note
Some coding features — including Codebase Context, code diffs, the code editor, and the file tree — are not yet available over SSH. See [Feature support over SSH](/terminal/code/ssh-feature-support/) for the full list.
:::

:::caution
This page is dedicated to the SSH features powered by `tmux`.

If you are looking to troubleshoot the legacy SSH implementation, see the [SSH (Legacy)](/terminal/terminal/warpify/ssh-legacy/).
:::
...
**Warpifying a remote SSH Session** [**will never make lasting changes to the remote machine without your explicit consent**](/terminal/terminal/warpify/ssh/#will-warpifying-a-remote-ssh-session-make-changes-to-the-remote-machine)**.**
...
Only to install [`tmux`](/terminal/terminal/warpify/ssh/#why-do-i-need-tmux-on-the-remote-machine) (a popular open source terminal multiplexer) and only with your explicit permission. If `tmux` is not installed, Warp will offer to install it for you and will show you the list of commands that will be run. You can always decline and continue to use your ssh session without some of Warp's features (or install `tmux` yourself and re-run Warpification [via the Command Palette](/terminal/terminal/warpify/ssh/#what-if-warp-fails-to-detect-my-ssh-session)).
...
If you are ever in a remote SSH Session and would like to manually Warpify, you can do so by using the [Command Palette](/terminal/terminal/command-palette/) and searching for "Warpify SSH Session".
...
If SSH Session Detection does not detect your session, you can still [Warpify manually](/terminal/terminal/warpify/ssh/#what-if-warp-fails-to-detect-my-ssh-session).
