...
:::note
If you are looking to troubleshoot the TMUX SSH feature, see the [SSH](/terminal/terminal/warpify/ssh/).
:::
...
:::note
Warp [Completions](/terminal/terminal/command-completions/completions/) for ssh show entries in `~/.ssh/config` and `~/.ssh/known_hosts`
:::
...
![Command SSH subshell workaround](/images/terminal/subshell-ssh-demo.gif)
