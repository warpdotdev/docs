...
* You can also toggle the quit warning feature in the [Command Palette](/terminal/terminal/command-palette/), by searching for \`Quit Warning'.
* If enabled, when you try and close Warp you will see a pop-up window with a few options listed below:
  ...
  * Show running processes, which will bring up the [Session Navigation](/terminal/terminal/sessions/session-navigation/) panel with a filter for running processes.
