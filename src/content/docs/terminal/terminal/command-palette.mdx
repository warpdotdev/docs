...
* Activate a specific filter, by clicking on the filter buttons or prepending your search with the following:
  * `workflows:` or `w:` will filter for [Workflows](/terminal/knowledge-and-collaboration/warp-drive/workflows/).
  * `prompts:` or `p:` will filter for [Prompts](/terminal/knowledge-and-collaboration/warp-drive/prompts/).
  * `notebook:` or `n:` will filter for [Notebooks](/terminal/knowledge-and-collaboration/warp-drive/notebooks/).
  * `env_vars:` will filter for [Environment Variables](/terminal/knowledge-and-collaboration/warp-drive/environment-variables/).
  * `files:` will filter for local files.
  * `drive:` will filter for [Warp Drive](/terminal/knowledge-and-collaboration/warp-drive/).
  * `actions:` will filter for Warp-specific actions like settings and features.
  * `sessions:` will filter for active sessions with [Session Navigation](/terminal/terminal/sessions/session-navigation/).
  * `launch_configs:` will filter for [Launch Configurations](/terminal/terminal/sessions/launch-configurations/).
