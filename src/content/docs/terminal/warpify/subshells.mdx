...
Commands that start an interactive SSH session, such as `ssh`, `gcloud compute ssh`, and `eb ssh`, go through [SSH Warpification](/terminal/warpify/ssh/) instead of the subshell flow described here.
...
Some types of subshells are not compatible, and you may also want to control Warp so it never invites you to "Warpify" the subshells for specific commands. When you add commands to the denylist, Warp will never invite you to "Warpify" subshells spawned by those commands.
