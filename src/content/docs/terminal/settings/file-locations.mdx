...
In addition, a small set of cross-cutting files always live under `~/.warp/` on every platform and channel — currently MCP server config and bundled skills. Stable and Preview share `~/.warp/` for these files (other channels like OSS use a suffixed directory such as `~/.warp-oss/`). They intentionally sit in the home directory so they're easy to reference from shell config, version control, or other tools.
...
If you run Warp on Windows and also use WSL, you can share Warp's config with your WSL `$HOME` so that tools running inside WSL can read the same themes, tab configs, settings, MCP servers, and agent config. WSL `$HOME` (typically `/home/<wsl-user>/`) is a separate filesystem from your Windows profile (`%USERPROFILE%`, typically `C:\Users\<Windows-user>\`), so nothing is shared automatically — every Warp directory you want to share needs its own symlink.

Windows splits Warp's data across four roots (Roaming AppData, Local AppData, and two home-directory paths) and your WSL Linux username is usually different from your Windows account name, so do this from a WSL shell:

```bash
# Set this once to your Windows username. Find it from WSL with:
...
ln -s "/mnt/c/Users/$WIN_USER/AppData/Roaming/warp/Warp/data" ~/.warp-windows-data

# Local config: settings.toml, keybindings.yaml (Local AppData).
ln -s "/mnt/c/Users/$WIN_USER/AppData/Local/warp/Warp/config" ~/.warp-windows-config

# Cross-platform carve-outs: MCP server config and bundled skills.
ln -s "/mnt/c/Users/$WIN_USER/.warp" ~/.warp

# Agent config (.agents).
ln -s "/mnt/c/Users/$WIN_USER/.agents" ~/.agents
```

If `~/.warp` or `~/.agents` already exist in WSL, remove or back them up before creating the symlinks. To resolve `%USERPROFILE%` programmatically instead of hard-coding the Windows username, use `wslpath "$(cmd.exe /C 'echo %USERPROFILE%' 2>/dev/null | tr -d '\r')"` from WSL.
