...
* `show_dock_icon` — Whether Warp is shown in the macOS Dock and `Cmd-Tab` switcher. Type: boolean. Default: `true`. macOS only.
...
* `enable_legacy_ssh_wrapper` — **Deprecated, migration-only.** No longer shown in Settings or used to gate behavior. Retained so Warp can run a one-time migration of a previously explicit value into `enable_ssh_warpification`. Type: boolean. Default: `true`.
* `use_ssh_tmux_wrapper` — **Deprecated, migration-only.** No longer shown in Settings or used to gate behavior. Retained so Warp can run a one-time migration that resets a previous opt-in and shows a deprecation notice pointing to the SSH extension. Type: boolean. Default: `false`.
