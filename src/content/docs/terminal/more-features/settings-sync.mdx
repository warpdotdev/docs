...
<figcaption>Settings Sync in Account pane.</figcaption>
</figure>

<figure>
![Settings Sync in Command Palette](../../../../assets/terminal/settings-sync-palette.png)
<figcaption>Settings Sync in Command Palette.</figcaption>
</figure>
...
That means your themes, most features, privacy settings, and AI settings are all the same everywhere you use Warp, saving you from having to set them up again.

When you first enable Settings Sync, the settings from the computer you enabled it on become the default settings for all devices. This is true if you toggle Settings Sync off and on as well — the synced settings are always from **the last device you enabled Settings Sync on**, so toggling effectively causes all of your devices to have settings from the currently logged-in instance.

:::note
Read more about privacy for cloud features in the [privacy overview](https://www.warp.dev/privacy).
:::
...
* Custom keybindings (we may support this in the future) — although you can set [custom keybinds with a file](/getting-started/keyboard-shortcuts/#custom-keyboard-shortcuts)
...
<figcaption>Indicator showing settings are not synced.</figcaption>
</figure>
