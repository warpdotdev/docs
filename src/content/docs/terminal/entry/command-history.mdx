...
While Warp is running, it isolates the history of each shell session. For example, if you have two Split Panes open, commands created in one pane don't populate the history of the other. Warp combines the history upon closing.
...
<figcaption>Command history with rich metadata.</figcaption>
</figure>

## How to access it

* Hitting `↑` in the [Input Editor](/terminal/editor/) brings up your history and performs a prefix search based on input.
