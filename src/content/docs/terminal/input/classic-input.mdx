...
Warp supports two input styles: **Shell (PS1)** and [Warp Input](/input/). Shell Input is closer to a traditional terminal experience, with support for shell customizations (e.g. Warp or PS1 prompt, same-line prompts, oh-my-zsh themes, and more).

You can switch between input styles in **Settings** > **Appearance** > **Input**.

:::note
Warp now uses [Agent Modality](https://docs.warp.dev/agent-platform/local-agents/interacting-with-agents/agent-modality), which provides a clean terminal by default and a dedicated conversation view for agent interactions. [Universal Input](/input/universal-input/) is a legacy option.
...
* [Prompt](/appearance/prompt/) — Use a fully customizable Warp prompt or your shell prompt, with support for PS1 and same-line prompts.
* [Input Position](/appearance/input-position/) — Choose where the input appears in Warp, including both the prompt and the command line.
...
* [Text Selection](/more-features/text-selection/) — Use smart selection or rectangular (column) selection to highlight text precisely without tedious cleanup.
...
* Toggle this feature **Settings** > **AI** > **Show input hint text** or search for "Input hint text" in the [Command Palette](/command-palette/) or Right-click on the input editor.
