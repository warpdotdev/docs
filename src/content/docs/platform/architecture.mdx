...
* **MCP credentials** - OAuth-connected servers use Warp's proxy. Non-OAuth URL-backed servers may use the proxy or, when Direct managed MCP is enabled, connect from the execution environment. Command-based servers always connect from the execution environment. Credentials for Warp-proxied servers stay with Warp; direct connections use local or managed secrets in the execution environment.
...
[Self-hosted execution](/platform/self-hosting/) moves the execution boundary: checkout, builds, and command execution stay on your infrastructure, and no Warp-hosted sandbox is involved. The control plane still coordinates runs, holds control-plane data, routes inference for the Warp Agent, and stores run data.
