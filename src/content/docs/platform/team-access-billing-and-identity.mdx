...
:::
   ...
   <figure>
   ![Oz by Warp GitHub App installation page showing repository access options](../../../assets/agents/oz-github-app-installation.png)
...
* **[GitHub integration](/platform/integrations/github/) runs** (an `@oz-agent` mention on an issue or pull request) - The agent authenticates as the installation that delivered the event, so its repository access and its GitHub attribution match the agent API key flow. In the Oz dashboard the run is still attributed to the teammate who wrote the mention, and their team is billed.
