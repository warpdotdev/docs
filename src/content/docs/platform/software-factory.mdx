...
The diagram below shows how a label change on the issue hands control from one agent to the next, and where the human review gate sits in that handoff:

```mermaid
flowchart LR
    Issue([New issue filed]) --> Triage["Triage agent"]
    Triage -->|"ready-to-spec"| Spec["Spec agent"]
    Spec -->|"PRODUCT.md + TECH.md"| Review{{"Human reviews\nand approves specs"}}
    Review -->|"implementation label"| Implement["Implementation agent"]
    Implement --> Reviewer["Reviewer agent"]
    Reviewer --> PR([Pull request for human merge])
```
