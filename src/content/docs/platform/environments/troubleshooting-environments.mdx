...
Setup commands run in a new container on every cloud agent run. Commands that depend on existing directories, caches, or cloned repositories can fail with [`environment_setup_failed`](/factories/api-and-sdk/troubleshooting/errors/environment-setup-failed/).
...
3. Follow [GitHub authorization setup](/agents/cli/oz-cli/integration-setup/#how-github-authorization-works) for the full flow.
...
3. If the error continues, review the environment configuration in the {VARS.WEB_APP} or use the {VARS.WARP_AGENT_CLI} to run `oz environment get <ENV_ID>`.
...
* [`environment_setup_failed`](/factories/api-and-sdk/troubleshooting/errors/environment-setup-failed/) for the API error reference.
