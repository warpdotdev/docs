...
:::caution
The Oz CLI has no **Agent** flag, so a schedule created with `oz schedule create` always runs as its creator. To run it as a cloud agent, set **Agent** on the schedule in the {VARS.WEB_APP} after creating it.
:::
