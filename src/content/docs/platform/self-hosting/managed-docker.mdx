...
There are three production-ready ways to install and run the worker with the Docker backend: as a Docker container, via Homebrew or Linuxbrew, or as a prebuilt binary from GitHub Releases. Docker is the recommended default; `go install` and building from source are for contributors and one-off testing.
...
### Option 2: Homebrew or Linuxbrew

Install the worker binary with [Homebrew](https://brew.sh/) (macOS) or [Linuxbrew](https://docs.brew.sh/Homebrew-on-Linux) (Linux) from the [`warpdotdev/warp` tap](https://github.com/warpdotdev/homebrew-warp):

```bash
brew install warpdotdev/warp/oz-agent-worker
```

Then run the binary directly — it uses the same Docker-daemon discovery described in [Docker connectivity](#docker-connectivity) below:
...
Download the archive for your platform from the [oz-agent-worker releases page](https://github.com/warpdotdev/oz-agent-worker/releases) (Linux, macOS, and Windows; amd64 and arm64), extract it, and run the binary:
...
For contributors, `go install github.com/warpdotdev/oz-agent-worker@latest`, or cloning the repository and running `go build`, also produce a working binary.
