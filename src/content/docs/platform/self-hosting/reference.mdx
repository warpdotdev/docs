...
Look up CLI flags and YAML config fields for the `oz-agent-worker` daemon's [managed backends](/platform/self-hosting/#managed-architecture). For installation, see [Install and run the worker](/platform/self-hosting/managed-docker/#install-and-run-the-worker).
