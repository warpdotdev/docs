...
* `target_dir` — Shared directory used as the working directory for every task. When set, the worker does not create or remove per-task workspace directories.
...
* [Direct backend monorepo worktrees](/platform/self-hosting/direct-monorepo-worktrees/) — Setup and teardown hooks for isolated monorepo worktrees.
