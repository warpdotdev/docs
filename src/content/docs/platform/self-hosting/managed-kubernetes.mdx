...
2. On startup, the worker runs a preflight Job using the worker's `pod_template`.
3. For each assigned task, the worker creates a Kubernetes Job from `pod_template` in the configured namespace. The task's runner supplies the image and, when configured, the instance shape.
