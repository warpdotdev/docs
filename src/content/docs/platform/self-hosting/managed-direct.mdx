...
Export the API key so the worker can authenticate to {VARS.WARP_AUTOMATION_PLATFORM}:
...
**Expected outcome:** The worker connects to {VARS.WARP_AUTOMATION_PLATFORM} and begins listening for tasks. Each assigned task runs in a freshly-created subdirectory of `workspace_root`.
