...
* **The `oz-agent-worker` binary** installed on the worker host. The Direct backend runs the worker itself on the host rather than in a container, so install it via [Homebrew/Linuxbrew or a GitHub Releases download](/platform/self-hosting/managed-docker/#install-and-run-the-worker) instead of the Docker option there.
