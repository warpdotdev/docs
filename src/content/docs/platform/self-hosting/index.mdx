...
The two architectures are not mutually exclusive. Some teams run managed workers for integration-triggered work and unmanaged agents in CI pipelines. For a visual comparison of what runs where in each model, see the deployment models diagram on [Deployment patterns](/platform/deployment-patterns/).
...
The numbered steps in the diagram are explained in the [self-hosted execution flow](/platform/architecture/self-hosted-execution/) reference, which also includes a step-by-step sequence view of a run.
