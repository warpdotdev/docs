...
* **Task isolation** — Each task runs as a separate Kubernetes Job/Pod. Successful Jobs are removed after completion. With cleanup enabled, failed Jobs and Jobs orphaned by worker disruption remain for 24 hours by default; use `--no-cleanup` to keep them without a TTL for debugging.
