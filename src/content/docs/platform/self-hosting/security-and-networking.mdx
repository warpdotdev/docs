...
The exception runs the other way. [Team-managed inference endpoints](/enterprise/enterprise-features/team-managed-keys-and-endpoints/) and Warp-proxied URL-backed MCP servers that you host must be reachable from Warp's servers. When Direct managed MCP is enabled and selected for a URL-backed server, the server must instead be reachable from the worker's execution environment.
