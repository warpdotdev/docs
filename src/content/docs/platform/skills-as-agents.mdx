...
description: >-
  ...
  with local or cloud agents from the CLI, {{WEB_APP}}, API, or on a schedule.
sidebar:
...
For reusable instructions scoped to a factory or one of its agents, use [factory skills](/factories/factory-skills/). This page describes skills that run independently of a factory.
...
3. **The skill appears** in the Agents list in the {VARS.WEB_APP}

:::note
You can also list available skills programmatically using the `GET /agent` endpoint. See the [{VARS.API_SDK_NAME}](/factories/api-and-sdk/) reference for details.
:::
...
Use the [{VARS.WEB_APP}](/platform/oz-web-app/) to run skill-based agents from a visual interface. From the web app, you can:
...
For a complete walkthrough of the web app interface, see [{VARS.WEB_APP}](/platform/oz-web-app/).

### CLI

Use the `--skill` flag with the {VARS.WARP_AGENT_CLI}:
...
For full CLI documentation, see [Using skills](/agents/cli/oz-cli/#using-skills) in the CLI reference.
...
For full API documentation, see [Agent configuration](/factories/api-and-sdk/#agent-configuration) in the API reference.
...
You can also create schedules from the [{VARS.WEB_APP}](/platform/oz-web-app/) using the **New schedule** action.
...
The [{VARS.WEB_APP}](/platform/oz-web-app/) displays suggested agents from the public [warpdotdev/oz-skills](https://github.com/warpdotdev/oz-skills) repository. These are pre-built skills that demonstrate common use cases and can be used as starting points for your own workflows.
...
* [{VARS.WEB_APP}](/platform/oz-web-app/) — Visual interface for managing cloud agents
* [{VARS.WARP_AGENT_CLI}](/agents/cli/oz-cli/) — Command-line interface for running agents
* [{VARS.API_SDK_NAME}](/factories/api-and-sdk/) — Programmatic access to cloud agents
