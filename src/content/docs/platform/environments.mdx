...
Environments define _how_ an agent runs, not _what_ it does. They're required for [Oz Platform](/platform/overview/) automation (cloud agents, integrations, API runs) but are not required for interactive local usage.
...
* [Multi-agent orchestration](/platform/orchestration/) - Spawn parent and child cloud agents that share environment configuration.
