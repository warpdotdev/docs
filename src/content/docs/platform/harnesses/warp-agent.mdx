...
* **Cross-harness orchestration parent** - A Warp Agent parent run can spawn Claude Code or Codex children and coordinate their outputs in [multi-agent orchestration](/platform/orchestration/). Other harnesses cannot act as parents in a multi-harness orchestration.
...
Warp Agent is the orchestration host for multi-harness runs. [Multi-agent orchestration](/platform/orchestration/) covers the parent/child model, run states, and patterns; a typical multi-harness pattern looks like:

1. A Warp Agent parent run analyzes a task and breaks it into subtasks.
2. The parent dispatches Claude Code children for steps that need careful review and Codex children for high-volume edits.
3. The parent collects results, resolves conflicts, and returns a single final output (a PR, a report, a Slack reply).

Children run in the same environment as the parent and share the same secrets, MCP servers, and integrations. The transcript shows the full tree, so reviewers can see exactly which child did what.
...
* [Multi-agent orchestration](/platform/orchestration/) — parent/child runs, including cross-harness children Warp Agent can spawn.
