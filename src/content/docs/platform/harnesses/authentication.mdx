...
Auth secrets can be scoped to a **team** (available to all teammates' runs) or **personal** (only your own runs), like any other Warp-managed secret. A [factory agent](/factories/factory-agents/#configuring-claude-code-or-codex-for-an-agent) can only select **team**-scoped secrets, since a factory's runs aren't tied to one person; personal secrets remain available when you start an ad hoc cloud run instead.
