...
See the [`oz federate issue-token` reference](/reference/cli/federate/#oz-federate-issue-token) for
token lifetime and subject customization options.
...
Use [`--subject-template`](/reference/cli/federate/#subject-template-components) to include other
available claims in `sub`.
