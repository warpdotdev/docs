...
```bash
oz federate issue-token --run-id RUN_ID --audience AUDIENCE --output-format json
```

Replace `RUN_ID` with the current agent run's ID and `AUDIENCE` with the identifier expected by
your provider.
...
Use `--subject-template` to build `sub` from one or more space-separated claim names. Components
appear in the order supplied and are separated by commas:
...
| Component | Value |
| --- | --- |
| `principal` | `user:USER_UID` or `service_account:SERVICE_ACCOUNT_UID`. Available for every token. |
| `scoped_principal` | `scoped_principal:TEAM_UID/user:USER_UID` for a user on one team, or `scoped_principal:user:USER_UID` for a user with no team. Service accounts always include the team UID. Unavailable for users on multiple teams. |
| `email` | The user's email address. Available for user tokens that include an email address. |
| `teams` | Team UIDs joined by `+`. Available when the principal belongs to at least one team. |
| `factory_uid` | The factory UID. Available for service accounts linked to a factory. |
| `agent_type` | The configured factory agent type. Available for service accounts with an agent type. |
| `run_id` | The current run ID. Available inside a cloud agent run with a valid workload token. |
| `environment` | The environment ID. Available when the run uses an environment. |
| `agent_name` | The configured agent name. Available when the run has a named agent. |
| `skill_spec` | The canonical skill identifier. Available when the run invokes a skill. |
| `host` | `warp` for Warp-hosted runs or the worker host for self-hosted runs. |
...
The `sub` claim identifies the principal that an agent is executing as. This is either a Warp
user or an autogenerated service account for team-scoped runs.
...
Tokens include claims that describe the user or service account:

* `user`: the user's UID. Included only for user tokens.
* `service_account`: the service account UID. Included only for service account tokens.
...
* `factory_uid`: the factory UID. Included only for service accounts linked to a factory.
* `agent_type`: the configured factory agent type. Included only for service accounts with an
  agent type.

### Run claims

The following claims are included when the value is available from a cloud agent run with a valid
workload token:
