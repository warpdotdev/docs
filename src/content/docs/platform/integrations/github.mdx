...
The repository that triggered the event is always cloned for the run, so an environment is optional. Configure one when runs need additional repositories, a specific Docker image, or setup commands. Any repository you add has to be covered by the same GitHub App installation as the triggering repository.
...
* **Every repository in the run must belong to the same installation.** The triggering repository and every repository in the configured environment have to be covered by the installation that delivered the event, and that environment has to be a GitHub environment. A run whose environment points at repositories in another organization, or at another provider, fails to start.
