...
* **Plan and credits** - Your team must be on a plan that supports integrations (Build, Max, or Business) with credits available. See [Access, billing, and identity](/platform/team-access-billing-and-identity/).
* **Oz by Warp GitHub App** - A GitHub organization owner installs the [Oz by Warp](https://github.com/apps/oz-by-warp) GitHub App on the organization or account that owns the repositories, granting it access to all repositories or a selected set.
...
* **A connected GitHub account** - Each teammate connects their GitHub account to Warp once, so Warp can identify who triggered the run.
...
<figcaption>Installing the Oz by Warp GitHub App.</figcaption>
</figure>
...
Each teammate connects their GitHub account so Warp can attribute a mention to the right Warp user and run with that person's permissions.
...
Two layers control what an agent can do in GitHub:

1. **The Oz by Warp GitHub App installation** - Determines which organizations and repositories Warp can read, and where the app can post comments. Change the repository selection at any time in your [GitHub settings](https://github.com/settings/installations).
2. **The triggering user's permissions** - Runs are attributed to the Warp account bound to the GitHub account that posted the mention, and the agent works with that person's GitHub access. Agents can't reach repositories the triggering user can't access.

Warp matches the GitHub account to a Warp account through the GitHub connection each user authorizes, not by email address. The triggering user must also be a member of the Warp team associated with the installation.
...
* **The agent replies as the GitHub App.** Progress updates and summaries are posted by Oz by Warp; pull requests and commits follow the GitHub authorization of the triggering user.
...
Confirm the triggering user has write access to the repository and that the Oz by Warp GitHub App installation includes it. Agents can read public repositories without authorization, but writing branches and pull requests requires both.
