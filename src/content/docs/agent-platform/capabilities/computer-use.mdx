...
Computer Use is **enabled by default** for Oz cloud runs. When starting a run from the {VARS.WEB_APP}, the Computer Use checkbox is pre-selected. When calling the API without specifying `computer_use_enabled`, Computer Use is on. You can disable or explicitly control it through the following entry points:

### {VARS.WEB_APP}

In the [{VARS.WEB_APP}]({VARS.WEB_APP_URL}), Computer Use is enabled by default when starting a run. To change it for a specific run or agent:
