...
Recordings appear in the conversation, on the Oz web run page, and can be attached to a pull request description when the agent uses the PR-artifacts path. They are captured entirely inside the sandbox and never include your local desktop or credentials.
