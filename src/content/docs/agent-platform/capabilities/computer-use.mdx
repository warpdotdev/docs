...
These recordings are produced by the agent as part of a run and can be surfaced in the run's outputs—for example, attached to a pull request body alongside the agent's summary. Recording happens inside the same isolated sandbox as the rest of the session and captures only the sandbox display, never your local machine.
