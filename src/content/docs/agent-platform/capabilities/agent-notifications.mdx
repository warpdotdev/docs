...
<figcaption>A toast notification in the corner of the Warp window signals that an agent task has completed. Click the toast to jump to the agent's session.</figcaption>
</figure>
...
<figcaption>The notification mailbox collects all agent notifications in one place. Use the filter tabs to switch between All tabs and Unread.</figcaption>
</figure>
...
<figcaption>Warp surfaces a one-click install chip in the agent utility bar when a supported CLI agent is running without the notification plugin installed.</figcaption>
</figure>
