...
If a recording fails to start, retry the run. Recording failures at startup are typically transient and resolve on the next attempt. If you encounter the same error repeatedly across multiple runs, file an issue on the [open-source Warp client repository](https://github.com/warpdotdev/warp/issues) with the error message and a description of your environment.
