...
1. **Enable Computer Use.** The agent needs Computer Use enabled for its run. See the [Computer Use](/agent-platform/capabilities/computer-use/#configuring-computer-use) page for how to configure it via the Oz web app, the CLI, or the API.
