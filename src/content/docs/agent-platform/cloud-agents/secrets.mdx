...
A [cloud environment](/agent-platform/cloud-agents/environments/) can declare its own list of secrets. When a run uses that environment, only those secrets are injected from the environment's contribution, regardless of every other team secret the caller has access to. Use this when a workflow's runtime needs a known, fixed set of credentials — for example, an `ops-tools` environment that only needs `DEPLOY_TOKEN` and `PAGERDUTY_API_KEY`.

Attach secrets to an environment from the environment form in the [Oz web app](/agent-platform/cloud-agents/oz-web-app/) or when creating an environment with the CLI. See [Environments](/agent-platform/cloud-agents/environments/) for the full configuration reference.
