...
Use this when you need execution and code to remain inside your network boundary, but still want Oz orchestration and visibility. Source code, build artifacts, and runtime secrets stay entirely on your infrastructure — only orchestration metadata and LLM inference route through Warp's backend.
...
* **Unmanaged** — Use `oz agent run` as a drop-in for your existing CI, Kubernetes, or dev environment. You control orchestration; Warp provides tracking.
...
* You need agents to access services behind a VPN or self-hosted SCMs like GitLab or Bitbucket.
...
For setup instructions and a decision guide, see [Self-Hosting](/agent-platform/cloud-agents/self-hosting/).
