...
Use this page when you need to audit a cloud agent run, debug a failed automation, or give teammates a shared record of what the agent did. The session is the review surface for the run: it shows the prompt, plan, commands, logs, outputs, and follow-up messages where available.
