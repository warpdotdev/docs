...
The **Agent** dropdown lets you choose **Quick run** or a saved agent before you start the run. **Quick run** runs as your own user. Your prompt adds context for this particular execution.
...
Saved agents are available in new run and schedule forms, so teams can reuse the same configuration without setting up each run from scratch. They use Warp's team-scoped identity model, which lets a saved agent own runs and carry default configuration for teammates. You manage saved agents from the Agents page; for the full reference, including plan limits, API endpoints, and API key binding behavior, see [Agent identities](/agent-platform/cloud-agents/agents/).
