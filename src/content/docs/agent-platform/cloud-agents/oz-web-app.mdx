...
5. **Base harness** — Choose the execution harness this identity uses by default (for example, the Oz harness or a supported third-party CLI agent).
...
9. **Memory stores** — When memory stores are available to your team, attach the ones this identity should read from or write to.
