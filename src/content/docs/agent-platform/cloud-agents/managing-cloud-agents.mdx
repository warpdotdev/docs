...
* Which session to open when you need to inspect the agent's prompt, plan, commands, logs, outputs, or follow-up messages
...
### Audit or review an agent run

Use the management view as the starting point when a teammate asks, "What did the agent do?"

1. Filter the list by source, day, creator, or status to find the run.
2. Open the run to view the shared session or local conversation.
