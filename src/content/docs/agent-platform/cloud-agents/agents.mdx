...
To create a key bound to a specific identity, choose the identity in the Oz web app's New API key dialog. See [API keys](/reference/cli/api-keys/) for the full key creation flow and for the difference between personal and agent keys, including the note on the legacy team-key label.
