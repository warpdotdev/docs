...
Read on to learn about the two loops that make up a software factory, the agent roles involved, and when this model is a good fit.
...
The outer improvement loop runs as a scheduled cloud agent. It collects signals from past inner-loop runs and generates a diff to the relevant skill files. That diff goes through a normal pull request review before merging. Humans decide what improves; agents propose it.
