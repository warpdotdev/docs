...
When a cloud session ends before the work is done, cloud-to-cloud handoff lets you continue the same run in a fresh cloud session. The run continues in the same conversation, with the prior session's workspace state restored, so the agent picks up where it left off instead of starting over.
...
You don't take a separate "hand off" action. Send a follow-up from the Warp app, and the run continues. The handoff happens automatically when the original session has ended.
