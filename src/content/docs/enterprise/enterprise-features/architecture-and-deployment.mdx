...
2. **Orchestration** - What decides what to run and tracks it ({VARS.WARP_AUTOMATION_PLATFORM} orchestrator or your own system).
...
* The {VARS.WARP_AUTOMATION_PLATFORM} orchestrator manages agent lifecycle - provisioning, execution, monitoring, and cleanup.
...
**Managed** - Run the `oz-agent-worker` daemon to let the {VARS.WARP_AUTOMATION_PLATFORM} platform orchestrate agents in isolated Docker containers on your infrastructure.

* The worker process connects to {VARS.WARP_AUTOMATION_PLATFORM} via WebSocket and receives tasks automatically.
...
* You want {VARS.WARP_AUTOMATION_PLATFORM} orchestration and visibility without sending code to Warp's infrastructure.
...
* Both execution modes share the same {VARS.DASHBOARD}, session sharing, and API/SDK visibility.
