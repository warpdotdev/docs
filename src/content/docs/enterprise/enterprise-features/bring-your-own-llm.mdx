...
1. **Admin configures routing** - Your team admin sets routing policies in Warp's admin settings (e.g., "Route Claude Sonnet 4.6 through AWS Bedrock; disable direct Anthropic API").
...
2. Select which models should use your cloud provider (e.g., "Claude Sonnet 4.6 via AWS Bedrock").
...
For example, if Claude Sonnet 4.6 on Bedrock fails but your admin also enabled it via direct API, Warp falls back to the direct API to avoid disruption. If a fallback uses a direct API model, that request consumes Warp credits.
