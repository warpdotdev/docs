...
* Restrict `sub` to your team's principals using the `scoped_principal:<team-uid>/*` pattern. This single condition covers both regular users running interactive or cloud agents and named-agent service accounts on the team.
  ...
  "Statement": [
    {
      ...
      "Condition": {
        "StringLike": {
          "${ISSUER_HOST}:sub": "scoped_principal:${TEAM_UID}/*"
        },
        "StringEquals": {
