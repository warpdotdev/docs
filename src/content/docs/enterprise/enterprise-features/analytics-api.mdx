...
Only **personal** Warp API keys created by an admin from **Settings** > **Cloud platform** > **API keys**. Agent API keys (including legacy team keys) are explicitly rejected by these endpoints. See [API Keys](/agents/cli/oz-cli/api-keys/) for how to create one.
...
* [API Keys](/agents/cli/oz-cli/api-keys/) - Create and manage personal Warp API keys.
