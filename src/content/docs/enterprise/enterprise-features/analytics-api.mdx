...
:::note[Early access]
The Enterprise Analytics API is in Early Access. Endpoints, query parameters, and response fields may change without notice. Access is granted to enterprise teams on request — contact your Warp account team to enable it for your team. After access is granted, an admin must turn on **Enterprise Usage Reporting (Early Access)** in **Admin Panel** > **Privacy** before any usage data is recorded.
:::
...
* **Enterprise plan with Analytics API access** - The Analytics API is gated to enterprise teams enrolled in Early Access. Contact your Warp account team to request access.
...
If your team has Early Access enabled and at least one admin has turned on the **Enterprise Usage Reporting (Early Access)** toggle, you'll get a JSON payload aggregating credit usage and conversation counts across the team. Continue to the [Endpoints](#endpoints) section for per-endpoint details.
...
In the grouped response, every leaf array is the same length as `period_list`, and the index of each value corresponds to the same index in `period_list`.
...
* `user_id` (optional) - Firebase UID of a single team member.
* `conversation_id` (optional) - Restrict results to one conversation.
* `message_type` (optional) - Filter by message variant. Common values include `agent_output`, `tool_call`, and `tool_call_result`. The full set mirrors the `Message.message` proto oneof, so additional values may appear over time. Internal scaffolding messages (for example `model_used`, `server_event`, `messages_received_from_agents`, `events_from_agents`, `debug_output`) are filtered out at write time and never appear here.
* `tool_type` (optional) - For `tool_call` / `tool_call_result` messages, restrict to a specific tool. Values are the snake_case proto field names of the tool (for example `apply_file_diffs`, `run_shell_command`, `search_codebase`, `grep`, `call_mcp_tool`).
...
* `llm_request_ids` and `model_ids` are arrays. Most LLM-backed messages have one entry; multi-partition summarization can list several.
...
* `code_changes` is omitted (or `null`) for messages that don't suggest or apply file changes. When present, `was_edited_by_user` is `true` when the user edited the diff before accepting, `false` when accepted as-is, and `null` when the message had no acceptance event.
...
The Analytics API is actively evolving. Expected near-term changes include:

* **Richer `git_context`** - Today `git_context` is populated with `head` and `branch` when available; expect additional repository identifiers (such as remote URL and pull request) over time.
* **Schema stabilization** - Field names and shapes may still change before the API leaves Early Access. Treat unknown fields as additive and code defensively for fields that may appear in future responses.
* **Endpoint expansion** - Additional aggregations and filters may be added based on Early Access feedback.
...
Only authenticated users with admin-level permissions on an enterprise team enrolled in Early Access. Calls from non-admin users return `403 Forbidden`.
...
### What's not in the data today?

Server-handled internal tool calls (such as `read_todos` and `list_messages_from_agents`) and internal scaffolding messages (such as `model_used`, `server_event`, `messages_received_from_agents`, `events_from_agents`, `debug_output`) are intentionally filtered out at write time — they're internal bookkeeping that never reaches the client and never appear in the events feed.
