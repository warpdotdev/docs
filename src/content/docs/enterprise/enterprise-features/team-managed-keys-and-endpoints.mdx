---
title: Team-managed API keys and endpoints
description: >-
  Configure shared provider API keys and custom endpoints for your whole team
...
* **Works for interactive sessions and cloud agents** - Because credentials are stored by Warp, team-managed providers power both interactive terminal requests and cloud agent runs. Self-serve, member-managed keys never leave the member's device and can't be used by cloud agents.
...
* **Optional member-managed keys** - Admins choose whether members may also add their own keys and endpoints on top of the team's.
* **Member key precedence** - When a member has their own key or endpoint for the same provider, their key is used instead of the team's.
...
For any provider or endpoint, a member's own key always takes precedence over the team's:
...
Configure team providers from the [Admin Panel](/enterprise/team-management/admin-panel/) **Models** page, where a **Bring Your Own Keys & Endpoints** card sits alongside the Direct API and AWS Bedrock cards.

1. In the [Admin Panel](/enterprise/team-management/admin-panel/), go to the **Models** page and find the **Bring Your Own Keys & Endpoints** card.
2. To add a team API key, enter the key for a provider (OpenAI, Anthropic, or Google) and save it. Each provider row shows whether a key is configured without revealing the stored value.
3. To add a team custom endpoint, enter a name, a base URL, an API key, and at least one model (a model name plus an optional alias), then save. An endpoint can't be saved without a name, a valid base URL, an API key, and at least one model.
4. Choose whether members may add their own keys and endpoints. When this is off, the member-facing self-serve controls are disabled, but any keys a member saved earlier are preserved for when it's turned back on.
...
Members don't configure anything for team providers. In their AI settings, a read-only **Provided by your team** group lists the team's configured providers and endpoints. For each first-party provider, members see whether a team key is configured and whether it's active or overridden by their own key — the stored key value is never shown.

In the model picker, team endpoint models appear as distinct entries labeled `Team · <endpoint name>`, and a member's own endpoints appear as `Custom · <endpoint name>`. The active model indicator makes the source clear: whether inference uses the member's key, the team's key, or Warp-managed inference.

If the admin allows members to add their own keys and endpoints, members also see the standard self-serve [BYOK](/agent-platform/inference/bring-your-own-api-key/) and [custom inference endpoint](/agent-platform/inference/custom-inference-endpoint/) controls under a separate group.
...
* **Platform credits still apply** - Local and cloud agent runs still consume [platform credits](/support-and-community/plans-and-billing/platform-credits/) for Warp's platform infrastructure (run lifecycle, orchestration, and observability), and cloud agent runs also consume compute credits.
