...
When you use agents in Warp, you're working with **Warp's built-in agents**. The {VARS.WARP_AUTOMATION_PLATFORM} is Warp's programmable system for running and coordinating agents at scale, whether they run locally on your machine or in the cloud.
...
The {VARS.WARP_AUTOMATION_PLATFORM} reads your codebase, understands its structure, and responds with a context-aware explanation.
...
* **Plan** - Use `/plan` to have the {VARS.WARP_AUTOMATION_PLATFORM} create a structured task plan for complex features
...
Monitor and steer cloud agents from the {VARS.DASHBOARD} or directly in Warp.
