...
:::note
This setting controls GitHub access for Agent API key runs only. Runs triggered by individual users (via `oz login`, the Warp app, Slack, or Linear) continue to use that user's personal GitHub token. For more details, see [Team GitHub authorization](/agent-platform/cloud-agents/team-access-billing-and-identity/#team-github-authorization).
:::
