...
* Fixed find (cmd+f) selection jumping to a different match when new output streams into the active block. Thanks @amazansky!
...
* Fixes issue with repeated TUI redraws for CLI agents on terminal pane resize.
...
* if user attaches an image in block input we should lock in agent mode, without running the NLD classifier to remove uncertainty
...
* Fixed scroll-to-start/end of selected block keybinding (Cmd/Ctrl+Shift+Up/Down) not working when the input is focused.
