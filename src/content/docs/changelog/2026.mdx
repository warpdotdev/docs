...
**{VARS.WARP_AUTOMATION_PLATFORM} updates**
...
* Replace {VARS.WARP_AUTOMATION_PLATFORM} armadillo icon with theme-adaptive Warp "W" logo (Icon::Agent) app-wide across all agent/{VARS.WARP_AUTOMATION_PLATFORM} interactive surfaces. ([#14344](https://github.com/warpdotdev/warp/pull/14344))
...
* Open recording artifacts in the authenticated viewer using the client-built {VARS.PLATFORM_RUN} URL. ([#14210](https://github.com/warpdotdev/warp/pull/14210))
...
* The run details Status chip now links directly to the run in {VARS.WARP_AUTOMATION_PLATFORM} web. ([#13455](https://github.com/warpdotdev/warp/pull/13455))
...
* Clearer Command Palette labels for installing/uninstalling the {VARS.WARP_AUTOMATION_PLATFORM} and Warp Control CLIs (they now describe global, outside-Warp use vs. in-Warp use). ([#13155](https://github.com/warpdotdev/warp/pull/13155))
...
* Fixed local {VARS.WARP_AUTOMATION_PLATFORM}/ambient agent runs reporting "Cancelled by user" when an agent-issued command exited the shell; the run now reports as failed with an explanation. ([#13210](https://github.com/warpdotdev/warp/pull/13210))

**{VARS.WARP_AUTOMATION_PLATFORM} updates**

* Added {VARS.WARP_AGENT_CLI} commands for memory store management: list/get/update stores, list store agents, list/create/update/delete memories, and view memory version history. ([#12973](https://github.com/warpdotdev/warp/pull/12973))
...
* Added a "Continue locally" button for in-progress cloud {VARS.PLATFORM_RUN}s so you can seamlessly switch to local execution. ([#12626](https://github.com/warpdotdev/warp/pull/12626)) — [@vkodithala](https://github.com/vkodithala) ✨
...
* The {VARS.WARP_AGENT_CLI} now has commands for operating over reusable agents. ([#11286](https://github.com/warpdotdev/warp/pull/11286))
...
* Added search to the {VARS.WARP_AUTOMATION_PLATFORM} Cloud API Keys settings page. ([#11307](https://github.com/warpdotdev/warp/pull/11307))
...
* Fixed broken {VARS.WARP_AUTOMATION_PLATFORM} notifications caused by a stale Claude plugin version. ([#11204](https://github.com/warpdotdev/warp/pull/11204))
...
* {VARS.WARP_AUTOMATION_PLATFORM} now refreshes GitHub credentials during task runs to keep Git operations authenticated. ([#11338](https://github.com/warpdotdev/warp/pull/11338))
* Add a View in {VARS.WARP_AUTOMATION_PLATFORM} action to orchestration child-agent pill menus. ([#11309](https://github.com/warpdotdev/warp/pull/11309))
...
* Run any agent harness in the cloud. Use {VARS.WARP_AUTOMATION_PLATFORM} to spin up Claude Code or Codex agents in the cloud; {VARS.WARP_AUTOMATION_PLATFORM} will help you track and steer the agents.
...
* Enabled {VARS.WARP_AUTOMATION_PLATFORM} OIDC authentication on stable ([#10468](https://github.com/warpdotdev/warp/pull/10468))
...
* Conversation details side panel is now available for local Warp Agent conversations, not just cloud {VARS.PLATFORM_RUN}s. Click the info button in the pane header to open it for any active AI conversation.
...
* Added `--version` flag support in the {VARS.WARP_AGENT_CLI}
...
* New AI setting to control whether {VARS.WARP_AUTOMATION_PLATFORM} adds an attribution co-author line to commit messages and pull requests it creates.
...
* Fixed "View in {VARS.WARP_AUTOMATION_PLATFORM}" button in the conversation details panel not responding to clicks.
* \[Windows] WSL panes in restored sessions have the same PWD when you exited.
* Restored the API Keys (BYOK) and AWS Bedrock sections in the Agents > {VARS.WARP_AUTOMATION_PLATFORM} settings subpage.
...
* Fixed {VARS.WARP_AGENT_CLI} hanging after command completes when the network is unavailable.
* Fixed Ctrl+C not terminating the {VARS.WARP_AGENT_CLI} during shutdown.
...
* Fixed a crash with {VARS.WARP_AGENT_CLI} commands that read from stdin.
...
* Fixed {VARS.WARP_AUTOMATION_PLATFORM} session sharing failing behind HTTP proxies on port 80.
...
* Updated settings copy to consistently use "{VARS.WARP_AUTOMATION_PLATFORM}" instead of "the Agent" in MCP Servers and Rules sections.
...
* `oz agent run` now prints the run ID with a link to the {VARS.DASHBOARD}.
...
* {VARS.WARP_AUTOMATION_PLATFORM} permission notifications now use "{VARS.WARP_AUTOMATION_PLATFORM}" branding instead of "Agent Mode".
* \[macOS] The window traffic light buttons now look correct at different zoom levels.
...
* {VARS.WARP_AGENT_CLI} now outputs a direct link to the {VARS.WARP_AUTOMATION_PLATFORM} webapp run page when spawning cloud agents.

**Bug fixes**

* Fixed a bug where the {VARS.WARP_AGENT_CLI} would open the Warp GUI app rather than printing help output.
...
#### Introducing {VARS.WARP_AUTOMATION_PLATFORM}: orchestration for cloud agents
...
{VARS.WARP_AUTOMATION_PLATFORM} is Warp's orchestration platform for cloud agents: launch parallel agents, automate recurring engineering work, and build apps on top of agents with full visibility and control.
...
* **Track agents from the web** — manage runs, create schedules, configure environments, and set up integrations from any browser in the <a href={VARS.WEB_APP_URL}>{VARS.WEB_APP}</a>.
...
* **Programmable by default** — orchestrate agents via the CLI and integrate {VARS.WARP_AUTOMATION_PLATFORM} into tools and services via the [API and CLI reference](/reference/).
...
* Fixed a race condition when using API keys with the {VARS.WARP_AGENT_CLI}.
