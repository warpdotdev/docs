...
An agent or automation can declare `computerUseModel` to override this default for its runs. Omitting the key or setting it to `null` inherits the default. The setting applies only to Computer Use on the Warp Agent harness (`type: oz`); Warp keeps but ignores it for a third-party harness or when Computer Use is disabled. It can appear alongside either `model` or `harness` because it doesn't select the agent's main model.
