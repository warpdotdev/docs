...
Warp publishes the definition format as JSON Schema (draft 2020-12) documents. Every field carries a description, and an unknown field is an error, so the schema is the exhaustive answer to what a file accepts. The endpoints are unauthenticated:
