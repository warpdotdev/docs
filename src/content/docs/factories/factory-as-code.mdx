...
Required. The repositories the factory works in, as `owner` and `name` pairs. The optional `codeForge` is `GITHUB` or `GITLAB`; omit it to use the factory's primary forge.
...
Optional. The forges connected to the factory: `GITHUB` or `GITLAB`. Omit the key to keep the stored set. An empty list disables all forges, and a list with more than one forge requires `codeForge` on every [`repositories`](#repositories) entry.
...
* Webhook: `webhook_ids` and `payload`. Payload patterns also accept `exists`.
