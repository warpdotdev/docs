...
* **In Warp** - The Warp Agent's built-in `factory-files` skill authors and validates definitions for you: ask it to change something in a factory repository or check the tree, and it edits against the current schema, then validates before opening a pull request. For example: "Add a nightly dependency-audit automation to this factory and validate the definition."
