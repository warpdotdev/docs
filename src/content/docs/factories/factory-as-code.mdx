...
description: >-
  Look up every file and key in a factory definition, fetch the JSON Schema
  behind them, and validate a change before it applies.
sidebar:
...
Definition files are YAML and Markdown. Keys are case-sensitive, and the YAML is plain: anchors, aliases, tags, and duplicate keys are rejected. The same rules are published as a [JSON Schema](#json-schema), the exhaustive reference for editors and coding agents.
...
When the definition lives in GitHub, every pull request that targets the production branch gets a **warp/factory-config** check. The check validates the head commit and dry-runs the change against your team, so it also catches a secret, runner, MCP server, or model that doesn't exist. It passes with a summary of what merging would create, update, and delete, or fails and annotates each problem on the line that caused it. A pull request that doesn't change the definition passes immediately. If the definition lives in a subdirectory of the repository, the directory is appended to the check name, as in **warp/factory-config (factory)**.

To stop an invalid definition from merging, make the check required: add **warp/factory-config** to the production branch's required status checks in your repository's branch protection rule or ruleset. Warp doesn't configure this for you.

When a change lands on the production branch, Warp applies it as a whole. A definition that fails validation never partially applies: the factory keeps running on its last valid definition until the branch is fixed. Warp-managed factories validate each edit when you save it in the web app instead, so their definitions can't become invalid.
...
* **In Warp** - The Warp Agent has a built-in `factory-files` skill for authoring definitions. Ask it to change something in a factory repository or to check the tree, and it fetches the current schema, edits against it, and runs the tree through the validation endpoint before opening a pull request, reporting the server's diagnostics rather than its own reading of the files. For example: "Add a nightly dependency-audit automation to this factory and validate the definition."
* **Through Factory MCP** - Connect any other coding agent to [Factory MCP](/factories/factory-mcp/) and it gets the same two operations as tools, `get_factory_file_schema` and `validate_factory_files`, along with the server's own guidance on using them. Neither needs a factory selected or a factory to exist. See [author and validate factory definitions](/factories/factory-mcp/#author-and-validate-factory-definitions).
* **Anywhere else** - An agent that can run a shell can call the validation endpoint or the script directly, the same way you would.
