...
The optional `agents` list pins named factory agents to the models and harnesses they use during that configuration's trials. Each entry requires `agent` and exactly one of `model` or `harness`, using the same model and harness forms as [`agentDefaults`](#agentdefaults). List an agent at most once per configuration. Agents omitted from the list use their configuration at launch time.
