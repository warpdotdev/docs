...
description: >-
  ...
  automations, routers, runners, benchmarks, scorers, skills, and webhooks.
sidebar:
  label: "Factory definition"
---
import { VARS } from '@data/vars';

Every factory is defined by files: a `factory.yaml` plus directories of agents, automations, routers, runners, benchmarks, scorers, skills, and webhooks, versioned in a Git repository. The files are the source of truth — when they change, Warp updates the factory to match.
...
For subdirectory check names and troubleshooting, see [factory-definition pull request checks](/factories/integrations/github/#factory-definition-pull-request-checks).

### Validate locally or in CI

Use the [`validate_factory_files.py`](https://github.com/warpdotdev/warp-factory-examples/blob/main/scripts/validate_factory_files.py) script to check a definition before you open a pull request or from your own CI. The script needs only Python 3 and doesn't require a Warp login or an existing factory.

```bash
python3 scripts/validate_factory_files.py path/to/factory-root
```

Run it against the factory root so it can check the complete definition. The script reports problems by file and line. A successful result means the files and their cross-file references are valid; the pull request check also checks settings that depend on your Warp team and current factory.
...
* **Through Factory MCP** - Connect any other coding agent to [Factory MCP](/factories/factory-mcp/) for the same schema and validation tools. See the [Factory MCP tool reference](/factories/factory-mcp/#tool-reference).
* **Anywhere else** - Have the agent run the [validator script](#validate-locally-or-in-ci).

## JSON Schema

Warp publishes the definition format as JSON Schema documents for editor completion and validation. Each field has a description, and the schema flags unknown fields. The endpoints are unauthenticated:
...
There is one document per file kind: `factory.schema.json` for `factory.yaml`; `agent.schema.json`, `automation.schema.json`, and `scorer.schema.json` for the frontmatter of the corresponding Markdown files; `router.schema.json`, `runner.schema.json`, `webhook.schema.json`, `benchmark_suite.schema.json`, and `benchmark_suite_task.schema.json` for the YAML files; and `common.schema.json` for the definitions the others share.
...
Optional. The code hosts enabled for the factory: `GITHUB` and `GITLAB`. Omit this key to use the factory's existing code host. To use repositories from both hosts, list both values and set `codeForge` on every repository. An empty `codeForges` list requires an empty `repositories` list and creates a factory with no code host.

### `repositories`

Required. The repositories the factory works in. Each entry requires `owner` and `name`; an optional `codeForge` selects `GITHUB` or `GITLAB`. Omit `codeForge` to use the factory's primary code host. When [`codeForges`](#codeforges) lists both hosts, every repository must declare `codeForge`.
...
Optional. Cloud-provider identity federation for agent runs. Under `cloudProviders.gcp`, use `projectNumber` (quote it so YAML keeps it a string), `workloadIdentityFederationPoolId`, `workloadIdentityFederationProviderId`, and the optional `serviceAccountEmail`. Under `cloudProviders.aws`, use `roleArn`.
...
For Linear, `linear.teamIds` limits issue discovery and routing to those team IDs. Omit it to include every team in the connected Linear workspace. For Jira, `jira.projectKeys` provides the equivalent scope; omit it to include every project on the connected Jira site.
...
For a third-party harness, `auth` sets where credentials come from: `source: managedSecret` reads the [managed secret](/platform/secrets/) named by `secretName`, and `source: workerEnvironment` reads them from your self-hosted worker's environment (and requires a self-hosted `workerHost`). The `oz` harness supplies its own credentials and takes no `auth` or `reasoningLevel`. The full nested paths are `agentDefaults.harness.type`, `agentDefaults.harness.model`, `agentDefaults.harness.reasoningLevel`, `agentDefaults.harness.auth`, `agentDefaults.harness.auth.source`, and `agentDefaults.harness.auth.secretName`.
...
MCP servers for agents that don't declare their own, in the same form as [`mcpServers`](#mcpservers). The nested paths are `agentDefaults.mcpServers.warpId`, `agentDefaults.mcpServers.command`, `agentDefaults.mcpServers.args`, `agentDefaults.mcpServers.env`, `agentDefaults.mcpServers.url`, and `agentDefaults.mcpServers.headers`. Like secrets, an agent that declares its own map replaces this one, and factory-wide servers always apply.

### `agentDefaults.workerHost`

Where runs execute: `warp` for Warp-hosted compute, or the ID of a connected worker in the [managed self-hosting architecture](/platform/self-hosting/#managed-architecture). Configure the worker's Docker, Kubernetes, or Direct backend on the worker itself; the factory definition selects the worker and a compatible runner.

### `agentDefaults.computerUseModel`

Optional. The `model_id` that Computer Use runs with. Omit it to let Warp select the model automatically.
...
Use `computer-use-agent-auto` for automatic selection. Pin a specific model when a benchmark or other repeatable workflow needs a fixed model. For models with multiple variants, the suffix selects the effort level; `thinking` enables thinking, and `xhigh-fast` selects fast mode.
...
| Claude Opus 5.5 | `claude-5-5-opus-low`, `claude-5-5-opus-medium`, `claude-5-5-opus-high`, `claude-5-5-opus-xhigh`, `claude-5-5-opus-xhigh-fast`, `claude-5-5-opus-max` |
...
* `spawnableBy` - Optional. The factory agents allowed to spawn this agent, named by their agent directory names. Omit it to allow only the foreman, use an empty list to allow no factory agent, or use a non-empty list as the exact allowlist. Include the foreman explicitly to retain its access.
...
* `idleTimeoutMinutes` - Optional. How long the run stays open after completion, from 1 through 60 minutes. Omit it or set it to `null` to inherit. Without an agent or run value, Warp uses 10 minutes, or 60 minutes for an orchestrated child run. A run-level value takes precedence.

The `harness` map accepts `harness.type`, `harness.model`, `harness.reasoningLevel`, `harness.auth`, `harness.auth.source`, and `harness.auth.secretName`. An agent's MCP map accepts `mcpServers.warpId`, `mcpServers.command`, `mcpServers.args`, `mcpServers.env`, `mcpServers.url`, and `mcpServers.headers`.
...
* `github` - `check_run_rerequested`, `check_suite_completed`, `check_suite_rerequested`, `issue_assigned`, `issue_created`, `issue_labeled`, `issue_mentioned`, `pull_request_assigned`, `pull_request_closed`, `pull_request_labeled`, `pull_request_mentioned`, `pull_request_merged`, `pull_request_opened`, `pull_request_ready`, `pull_request_reopened`, `pull_request_review_requested`, `pull_request_review_submitted`, `pull_request_synchronized`, `push`, `workflow_run_completed`
...
These dotted reference paths make the nested matcher shapes explicit. In a definition, write an allowed authoring path as nested YAML rather than as a dotted key:

* **Include and exclude** - `triggers.filter.actions.in`, `triggers.filter.actions.not_in`, `triggers.filter.assignee_ids.in`, `triggers.filter.assignee_ids.not_in`, `triggers.filter.assignees.in`, `triggers.filter.assignees.not_in`, `triggers.filter.authors.in`, `triggers.filter.authors.not_in`, `triggers.filter.baseBranches.in`, `triggers.filter.baseBranches.not_in`, `triggers.filter.base_branches.in`, `triggers.filter.base_branches.not_in`, `triggers.filter.branches.in`, `triggers.filter.branches.not_in`, `triggers.filter.channel_ids.in`, `triggers.filter.channel_ids.not_in`, `triggers.filter.conclusions.in`, `triggers.filter.conclusions.not_in`, `triggers.filter.creator_ids.in`, `triggers.filter.creator_ids.not_in`, `triggers.filter.emojis.in`, `triggers.filter.emojis.not_in`, `triggers.filter.issue_ids.in`, `triggers.filter.issue_ids.not_in`, `triggers.filter.item_user_ids.in`, `triggers.filter.item_user_ids.not_in`, `triggers.filter.keywords.in`, `triggers.filter.keywords.not_in`, `triggers.filter.labels.in`, `triggers.filter.labels.not_in`, `triggers.filter.mentioned.in`, `triggers.filter.mentioned.not_in`, `triggers.filter.mentioned_user_ids.in`, `triggers.filter.mentioned_user_ids.not_in`, `triggers.filter.paths.in`, `triggers.filter.paths.not_in`, `triggers.filter.prNumbers.in`, `triggers.filter.prNumbers.not_in`, `triggers.filter.pr_numbers.in`, `triggers.filter.pr_numbers.not_in`, `triggers.filter.project_ids.in`, `triggers.filter.project_ids.not_in`, `triggers.filter.project_keys.in`, `triggers.filter.project_keys.not_in`, `triggers.filter.repos.in`, `triggers.filter.repos.not_in`, `triggers.filter.review_states.in`, `triggers.filter.review_states.not_in`, `triggers.filter.reviewer_teams.in`, `triggers.filter.reviewer_teams.not_in`, `triggers.filter.reviewers.in`, `triggers.filter.reviewers.not_in`, `triggers.filter.stages.in`, `triggers.filter.stages.not_in`, `triggers.filter.state_ids.in`, `triggers.filter.state_ids.not_in`, `triggers.filter.status_ids.in`, `triggers.filter.status_ids.not_in`, `triggers.filter.team_ids.in`, `triggers.filter.team_ids.not_in`, `triggers.filter.user_ids.in`, `triggers.filter.user_ids.not_in`, `triggers.filter.workflows.in`, and `triggers.filter.workflows.not_in`.
* **Include only** - `triggers.filter.webhook_ids.in`.
* **Server-managed include only** - `triggers.filter.schedule_ids.in`.
* **Webhook payloads** - `triggers.filter.payload.exists`, `triggers.filter.payload.in`, and `triggers.filter.payload.not_in`.
...
An automation may also declare `model` or `harness`, `runner`, `environmentId`, `secrets`, `mcpServers`, `workerHost`, `computerUseModel`, and `idleTimeoutMinutes` to override the target agent's settings for the runs it starts. The timeout has the same 1-to-60-minute range and inheritance behavior as the [agent field](#agentsnameagentmd).

The automation's `harness` map accepts `harness.type`, `harness.model`, `harness.reasoningLevel`, `harness.auth`, `harness.auth.source`, and `harness.auth.secretName`. Its MCP map accepts `mcpServers.warpId`, `mcpServers.command`, `mcpServers.args`, `mcpServers.env`, `mcpServers.url`, and `mcpServers.headers`.

## `routers/<slug>.yaml`

Optional. Each file defines a factory-owned custom model router. The file name without `.yaml` is the stable router slug. Use `model: router:<slug>` in `agentDefaults`, an agent, or an automation to select it on the Warp Agent harness.

Both router types require `type` and `default`. Optional `name` is a display label. Every model target must be a concrete model ID, not an Auto model or another router.

For `type: complexity`, `routing` is an optional map with `easy`, `medium`, and `hard`. Any omitted level uses `default`.

```yaml title="routers/cost-aware.yaml"
name: Cost-aware
type: complexity
default: claude-5-sonnet-high
routing:
  easy: claude-5-fable-high
  medium: claude-5-sonnet-high
  hard: claude-5-opus-high
```

For `type: prompt`, `routing` is an ordered list. Each item requires `description` and `model`; the first matching description wins, and `default` applies when no item matches.

```yaml title="routers/by-task.yaml"
type: prompt
default: claude-5-sonnet-high
routing:
  - description: Debugging production incidents
    model: claude-5-opus-high
  - description: Drafting documentation
    model: claude-5-fable-high
```

See [custom routers](/agents/inference/custom-routers/) for routing behavior and model availability.
...
setupCommands:
  - corepack enable
failureSessionRetentionMinutes: 15
instanceShape:
  vcpus: 4
  memoryGb: 8
platform:
...
Optional. An integer from `1` through `60` that keeps a failed run's session open for inspection. Input from a shared-session viewer restarts this idle window. There is no runner-level default: the runner value overrides the run environment's setting, while an omitted value uses the environment's setting or tears down the session immediately when neither is set.

The value must not exceed the maximum runtime available to the sandbox. Warp disables retention for a run when the value exceeds that limit, and the sandbox's runtime limit can end the session before the full idle window elapses. Secrets remain in the sandbox, and compute credits may apply while the session stays open.

On a self-hosted worker, an agent client that does not support failure-session retention ignores the setting, so the session tears down after failure.

### `platform`

The operating system and architecture. `os` is `linux` (the default), `macos`, or `windows`. The supported pairs are Linux with `aarch64` or `x86_64`, macOS with `aarch64`, and Windows with `x86_64`. Linux runners require `linux.dockerImage`, the container image the sandbox boots, so every Linux runner declares a `platform` section. macOS runners accept an optional `platform.mac` map with `platform.mac.version` (`"14"`, `"15"`, `"26"`, or `"27"`; quote it, and it defaults to `"26"`). Windows runners don't accept a `linux` or `mac` map.
...
Optional. This file defines a [benchmark suite](/factories/benchmarks/): one agent, optional reusable configurations, and an ordered set of reusable tasks. The suite slug is its stable file location, not its display name, so renaming `name` does not move the file or change the suite's identity.
...
Optional. One to six reusable configurations for benchmark runs. When a run doesn't supply its own configurations, Warp uses these presets. Each configuration requires a unique `name` and a `role` of `baseline` or `candidate`; a suite can have at most one baseline.

The optional `agents` list pins named factory agents to the models and harnesses they use during that configuration's trials. Each entry requires `configurations.agents.agent` and exactly one of `configurations.agents.model` or `configurations.agents.harness`. A harness override requires `configurations.agents.harness.type` and `configurations.agents.harness.model`. It optionally accepts `configurations.agents.harness.auth`, with `configurations.agents.harness.auth.source` and `configurations.agents.harness.auth.secretName`. List an agent at most once per configuration. Agents omitted from the list use their configuration at launch time.
...
Optional. An ordered list of string tags. Omit it or use an empty list for no tags.
...
Optional. The starting Linear workspace state for an isolated benchmark trial. It accepts any JSON-compatible value, which the Linear sandbox interprets.
...
The provider signature to verify: `github`, `pagerduty`, `sentry`, `standard_webhooks`, `stripe`, or `vercel`. Required when `authMode` is `signature`, and invalid otherwise.
...
Optional. Defaults to `true`. Set it to `false` to stop deliveries immediately without deleting the webhook, or to create a webhook before its provider has issued a signing secret; see [Setting up a Vercel webhook](/factories/webhooks/vercel/).
