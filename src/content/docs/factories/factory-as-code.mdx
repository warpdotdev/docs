...
Optional. Cloud-provider identity federation for agent runs. The `cloudProviders.gcp` map accepts `cloudProviders.gcp.projectNumber` (quote it so YAML keeps it a string), `cloudProviders.gcp.workloadIdentityFederationPoolId`, `cloudProviders.gcp.workloadIdentityFederationProviderId`, and an optional `cloudProviders.gcp.serviceAccountEmail`. The `cloudProviders.aws` map accepts `cloudProviders.aws.roleArn`.
...
The legacy `providers` key is accepted when reading older definitions. Its nested keys are `providers.gcp`, `providers.gcp.projectNumber`, `providers.gcp.workloadIdentityFederationPoolId`, `providers.gcp.workloadIdentityFederationProviderId`, `providers.gcp.serviceAccountEmail`, `providers.aws`, and `providers.aws.roleArn`. Use `cloudProviders`; when Warp rewrites the definition, it emits `cloudProviders`.
