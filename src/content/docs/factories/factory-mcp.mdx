...
:::note
**Task** is this page's word for a factory's work item, because the MCP tools are named for it (`send_task`, `get_task`, `complete_task`). It is the same unit of work [how Warp Factories work](/factories/how-factories-work/) calls a work item, not a second kind of thing.
:::
...
After the user pastes the prompt:
...
* **Warp:** Factory MCP is built in. If you're running on the Warp harness and/or already have Factory MCP installed, skip MCP installation and go straight to onboarding.
* **Claude Code:** A server added after the session starts is not available to that running session. After adding Factory MCP, tell the user to exit and resume the same conversation from the same directory with `claude --continue`. Once resumed, complete authentication through the client's MCP controls if needed, then continue without asking the user to restate their goal.
* **Codex:** Add Factory MCP as a remote HTTP server, use Codex's built-in OAuth flow, and confirm the server is active through its MCP controls before onboarding. Ask the user to restart or resume only if the configured server is unavailable to the current session.
...
In both examples, `warp-factory` is just the name your client files the server under, and you can call it whatever you like. It doesn't name a particular factory, and it has nothing to do with your factory's name or its [Foreman name](/factories/factory-agents/#foreman-name). A single connection reaches every factory you have access to.
...
* "Add a review agent to this factory definition and validate it."
...
## Author and validate factory definitions

Two tools cover editing a factory's [definition files](/factories/factory-as-code/) in a repository. Neither takes a factory ID, and both work before the factory exists:

* `get_factory_file_schema` returns the JSON Schema for the definition format. Called with no arguments, it lists the available documents (`factory.schema.json`, `agent.schema.json`, and so on), the file each one governs, and a stable URL for each; called with `document`, it also returns that document's body. Warp owns the format, so agents read it from here instead of carrying a copy that would drift from what the server accepts.
* `validate_factory_files` validates a complete tree without saving or applying it. The agent submits every definition file as a `path` and its `content`, and the server runs the same parser it uses to apply a definition, returning `valid`, a list of `diagnostics` with a path, line, column, and `FF_*` code, and the state-dependent checks it did not run. The whole tree goes in every call, not only the changed file, because rules such as "exactly one foreman" span files.

A clean result means the tree parses and passes every check that doesn't need your team's state. Whether a secret, runner, model, or MCP server exists is settled when the change is applied, or by the **warp/factory-config** check on a GitHub-backed factory. A prompt like "Add a review agent to this factory definition and validate it" is enough: the agent reads the schema, edits, validates, and reports the server's diagnostics before opening a pull request. For the pull request check, the validation endpoint, and editor schema support, see [Validate a definition](/factories/factory-as-code/#validate-a-definition).
