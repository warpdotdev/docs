...
2. Add `https://app.warp.dev/api/v1/mcp/factory` using its own MCP configuration mechanism.
...
Some MCP clients load newly added servers only when a new session starts. If a reload is required, the agent will tell you how to resume onboarding after the server becomes available.
