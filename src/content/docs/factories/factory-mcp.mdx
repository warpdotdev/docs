...
The onboarding tools from `list_teams` through `create_factory` require browser sign-in or a personal API key. Agent API keys are for operating an existing factory and cannot use these setup tools.
...
| `list_tracker_scopes` | Lists Linear teams or Jira projects available for scoping which of them route issues to a new factory. |
