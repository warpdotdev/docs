...
* **Warp-provided platform-bundled skills** - Product capabilities that Warp makes available when the run supports them, such as `oz-platform`, `factory-files`, and `factory-mcp`.
...
The bundled set available to a run depends on the Warp release, release channel, enabled product features, required files, and connected integrations. See the [bundled skill ID catalog](/reference/api-and-sdk/#skill-actions-in-conversation-data) for the authoritative list of stable IDs and availability details.
...
* **Only `input.skill_path` is present** - The value is the path used to resolve the skill. It can identify a file-based skill or a path-referenced bundled skill, including one from a remote host. Count it as team-owned only when the path is under a skill root your team manages, such as `skills/` or `agents/<name>/skills/` in the factory definition.
