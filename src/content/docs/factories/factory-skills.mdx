...
Factory role and integration skills are separate from [skills bundled with Warp](/reference/api-and-sdk/#skill-actions-in-conversation-data). Platform-bundled skills include `oz-platform`, `factory-files`, and `factory-mcp`; their availability follows the Warp release and enabled product features. In Conversation API data, a `read_skill` action with `bundled_skill_id` identifies a platform-bundled skill.
