...
Work flows between the planes in both directions. Integrations and triggers hand work to the control plane, which dispatches each task, its configuration, and scoped runtime credentials to the execution host. Results, transcripts, attachments, artifacts, and telemetry flow back through the control plane, which also routes model requests to Warp-managed or customer-configured inference and persists run data to Warp or customer-owned storage.
