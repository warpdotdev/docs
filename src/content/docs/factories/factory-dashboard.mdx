...
The factory dashboard is the web app for operating a single factory. Open the <a href={VARS.FACTORY_WEB_APP_URL}>{VARS.FACTORY_WEB_APP}</a>, then select a factory to track its work, inspect its runs and pull requests, and manage its agents, automations, and settings.
...
* [Warp Factories benchmarks](/factories/benchmarks/) - Create a benchmark suite and compare configurations on fixed tasks.
