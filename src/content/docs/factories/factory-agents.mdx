...
Agent configuration can also be stored as version-controlled code: role instructions, model and harness choices, and access boundaries all live in a repo, where changes are reviewed like any other change and are easy to roll back. See [factory definitions as code](./factory-as-code) for how to set this up.
