...
2. Click an agent to open its settings, where you can change its description, harness, model, runner, host, [MCP servers](/platform/mcp/), [secrets](/platform/secrets/), and instructions.
...
You can also manage the whole factory as version-controlled code, with [factory definition files](/factories/factory-as-code/) in a Git repository. A Warp-managed factory edits every setting, including harness, auth, and credential strategy, from the dashboard; a GitHub-backed factory shows those same settings read-only, since its files are the only way to change them. See [where the definition lives](/factories/factory-as-code/#where-the-definition-lives) for the full difference.
...
1. Open the agent's settings (see [Configure agent behavior](#configure-agent-behavior)) and choose **Claude Code** or **Codex** in the **Harness** field. A harness your plan or workspace doesn't allow shows "This harness is not available on your plan. Switch to Warp to continue."; you can only move it back to the Warp Agent until an admin re-enables the harness or your team upgrades.
2. Choose or create a credential in the **Auth** field that appears below **Harness**. The dropdown lists only compatible, team-owned secrets: Claude Code accepts an Anthropic API key, an Anthropic Bedrock API key, or an Anthropic Bedrock access key; Codex accepts an OpenAI API key. Pick an existing secret, or choose **New auth secret** (**New OpenAI API Key** for Codex) to store one without leaving the page. See [connecting Claude Code credentials](/platform/harnesses/authentication/#connecting-claude-code-credentials) and [connecting Codex credentials](/platform/harnesses/authentication/#connecting-codex-credentials) for how to obtain each key. When the agent's host is a self-hosted worker, **Auth** also offers **From worker environment**, which reads the credential from that worker instead of a managed secret. Switching the host back to Warp-hosted compute clears this selection, so switch hosts first, then choose a compatible team-owned managed secret in **Auth**.
3. Choose a **Model**. The options come from the selected harness's own catalog, not Warp's, so changing harness always clears the model and auth you had selected; reselect both afterward. A Codex model option pairs a model with a reasoning level (for example, `gpt-5.5 (high)`), so pick the combined option rather than setting reasoning separately; Codex has no `default` entry that picks a model automatically.
4. Click **Save changes**. The agent's settings page shows the saved **Harness**, **Auth**, and **Model** values, confirming the agent is now configured to run on the third-party harness.

A Warp-managed factory edits harness, auth, and model from the dashboard. A GitHub-backed factory sets them instead through [`agentDefaults.harness` or a per-agent `harness` override](/factories/factory-as-code/#agentdefaultsharness) in the definition files. For a definition that gives each agent its own harness, with managed-secret auth for Claude Code and Codex, see [`03-multi-harness`](https://github.com/warpdotdev/warp-factory-examples/tree/main/examples/03-multi-harness).
