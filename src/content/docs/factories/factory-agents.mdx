...
The foreman's `run_agents` tool provides a `Factory sibling agents` roster. To select a named agent, set `remote` and copy its exact roster UID into `agent_run_configs[].agent_identity_uid`. Every child in that batch needs a roster UID.
