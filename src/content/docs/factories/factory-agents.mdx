...
3. In the **Auth** field, choose a compatible, team-owned secret from the list or click **New auth secret** to create one.
   ...
   Claude Code accepts an Anthropic API key, Anthropic Bedrock API key, or Anthropic Bedrock access key. Codex accepts an OpenAI API key. See [connecting Claude Code credentials](/platform/harnesses/authentication/#connecting-claude-code-credentials) and [connecting Codex credentials](/platform/harnesses/authentication/#connecting-codex-credentials) for how to obtain each key.
...
### Dispatch named factory agents

The foreman's `run_agents` tool lists the other agents in the factory under `Factory sibling agents`. To select one, set `remote` and copy its exact UID into `agent_run_configs[].agent_identity_uid`. Every child in that batch needs an agent UID.

The child `name` is only a display label. The selected agent supplies its environment, runner, host, harness, and model unless the call overrides them.

To spawn ad hoc children without standing configuration, use [multi-agent orchestration](/platform/orchestration/) from any Warp conversation.
