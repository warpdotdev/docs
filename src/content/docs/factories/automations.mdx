...
* **Keys AND together, and an omitted key matches everything.** `{"action": ["opened"]}` matches any delivery whose `action` is `opened`, whatever else the body contains.
* **A bare array means any-of.** `["opened", "reopened"]` matches either value. Values are JSON scalars: strings match exactly and case-sensitively, numbers match numerically (`1` equals `1.0`), and booleans and `null` are matchable values.
* **An operator object refines one field.** `in` is the same as a bare array, `not_in` excludes values, and `exists: true` or `exists: false` requires the key to be present or absent. Operators on the same field AND together.
* **Objects nest.** A nested pattern object descends into a nested payload object. When the payload value is an array of objects, the nested pattern matches if any element matches; when it's an array of scalars, `in` matches on any overlap and `not_in` requires none.
...
A pattern nests up to 10 levels deep, holds up to 64 leaf fields, and lists up to 32 values per set. Warp rejects a pattern that lists the same value in both `in` and `not_in` on one field, or that combines `exists: false` with a non-empty `in`. A filter that can't be evaluated never matches, so a broken filter keeps an automation silent rather than firing it.
