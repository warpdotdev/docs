...
5. Toggle the subagents the foreman can dispatch: **Triage**, **Spec**, **Code**, and **Review**. All four start enabled, and at least one is required. Leave **Code** on so this quickstart can end in a pull request. See [factory agents](/factories/factory-agents/) for what each does.
6. Optionally connect a chat tool and an issue tracker so teammates can hand work to the factory from where they already work, or skip both and add them later from [connect your factory](/factories/connect-your-factory/).
