...
A dispatched run is an ordinary [cloud agent run](/platform/): retrieve it, send it follow-ups, or cancel it through the same [Agent API](/reference/api-and-sdk/) you'd use for any run. See [migrating from `agent_identity_uid`](#migrating-from-agentidentityuid) if your integration dispatches to factories today by looking up their foreman agent directly.
