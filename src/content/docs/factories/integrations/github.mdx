...
* **The Warp GitHub App** - Follow the [GitHub integration setup](/platform/integrations/github/) to install the app and grant it access to the repositories the factory works in. The same installation covers factories if your team already set it up for the platform integration.
* **A factory with GitHub repositories** - At least one of the factory's repositories must also be one the Warp GitHub App has access to.
...
Managed GitHub factories start with two editable default automations: one for agent mentions and assignments (see [Mention the factory](#mention-the-factory)) and one that closes or merges pull requests. When Linear or Jira is connected, this automation can also complete the linked ticket after a merge.
...
The handle and label are default filter values, not fixed rules. Edit the automation's **Mentioned users or teams** filter to respond to other handles, such as your own `@org/team` slug, or remove the label filter to catch every mention in its repositories.
...
* **The app installation decides what agents can reach.** A run can only read and write the repositories and permissions your team granted the Warp GitHub App at install time. Selecting repositories for a factory or tightening automation filters changes when work starts, not what a running agent can access. To change access, change the installation.
