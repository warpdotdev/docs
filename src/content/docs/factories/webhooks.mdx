...
Vercel, Stripe, and PagerDuty only show a webhook's signing secret after you save its URL in their dashboard, and Warp only has a URL once the webhook exists. To break the loop, create the webhook without a secret: Warp creates it disabled, gives you the URL, and takes the secret in a second step. The steps below use Vercel; Stripe and PagerDuty work the same way.

Vercel team webhooks require a Vercel Pro or Enterprise plan.

### In the factory dashboard
...
3. Leave "Provider secret" blank and keep the suggested "Secret name". Click **Create and get URL**.

   The pane changes to **Finish webhook setup** and shows the ingress URL. The webhook exists but is disabled, so it accepts no deliveries yet.
4. In the Vercel dashboard, go to your team's **Settings** > **Webhooks** and click **Create Webhook**. Choose the events and projects, enter the ingress URL as the "Endpoint URL", and click **Create Webhook**. Vercel shows the webhook's secret once.
5. In Warp, paste that secret into "Secret from Vercel" and click **Save and enable**.

The webhook is enabled and verifies the `x-vercel-signature` header on every delivery. Trigger a deployment and confirm it appears under **Recent deliveries** as **Accepted**.

To finish later, click **Finish later** instead. The webhook stays disabled: open it from the **Webhooks** page, supply the secret (click **Rotate secret** and paste it, or on a Warp-managed factory update the managed secret on the team's **Secrets** page), then toggle **Enabled** on.

### In a definition that lives in GitHub
...
1. On the team's **Secrets** page, create a secret such as `VERCEL_WEBHOOK_SECRET` with any placeholder value. A webhook file must name a secret that already exists.
...
3. Create the webhook in Vercel with that URL, as in step 4 above, then update `VERCEL_WEBHOOK_SECRET` on the **Secrets** page with the secret Vercel shows.
4. Change `enabled: false` to `enabled: true` and merge. Applying this change reads the new secret value and enables the webhook.

Creating the webhook with `enabled: true` from the start rejects every delivery with `401` until another change lands after you update the secret.
...
On a Warp-managed factory, the dashboard writes each webhook to a definition file, so two actions work differently: rename isn't supported, and **Rotate secret** is replaced by rotating the managed secret the webhook references on the team's **Secrets** page. The webhook picks up the new value the next time its definition applies: any change to the webhook in the dashboard, such as toggling **Enabled**, or for a GitHub-backed factory the next change merged to the production branch.
