...
| [Jira](./integrations/jira) | Work items assigned to Warp through Rovo | The Rovo agent session | Task status and a text result |
...
* [Jira](./integrations/jira) - Route work items assigned to Warp in Jira through Rovo agent sessions.
...
* **Follow-ups continue the same work item** - Replying in the same Slack thread, GitHub issue or pull request, Linear issue, or Jira Rovo agent session adds to the existing work item instead of starting a new one. Repeated event deliveries from a provider don't create duplicate work either.
