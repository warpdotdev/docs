...
## Review seeded default automations

When you create a managed factory, Warp seeds provider-aware default automations for connected providers.

| Provider | Seeded starting point |
| --- | --- |
| GitHub | Routes mentions and assignments into factory work, then handles pull requests when they close or merge. When a pull request is linked to tracker work, this handling can complete the tracker item when applicable. |
| Jira | Routes agent sessions from the selected Jira projects into factory work. |
| Slack | Uses the default automations described in the [Slack integration guide](./integrations/slack). |

Treat these automations as starting points. Review and edit their subscriptions, filters, agents, and run settings to match your workflow.
