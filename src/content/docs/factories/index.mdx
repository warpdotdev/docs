...
A factory and its foreman are easy to conflate, partly because setup gives them the same name by default. Factory MCP adds a third, unrelated name into the mix.
...
* **Factory MCP** - A hosted MCP server that connects a coding agent to your factories, so it can send work in or pull work down to finish locally. It's a way into a factory, not a factory itself, and the client-side name `warp-factory` is something you choose, unrelated to any factory or foreman name. See [Factory MCP](/factories/factory-mcp/).
