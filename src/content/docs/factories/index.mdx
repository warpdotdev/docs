...
* **Foreman name** - The handle your team @-mentions in Slack and Linear, such as `@payments`. Setup copies it from the factory's name, so the two usually match, but what you mention is the foreman, which speaks for the factory. In the factory dashboard, change it under **Settings** > **Identity**, or use the [`alias`](/factories/factory-as-code/#alias) key in the definition.
