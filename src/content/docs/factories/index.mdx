...
A factory, the foreman that runs it, and the Factory MCP server are distinct things that are easy to conflate, partly because setup gives the first two the same name.

* **Warp Factories** - The product, always plural and written in full. One deployment of it is a *factory*, lowercase.
* **Factory** - A single deployment: its repositories, agents, integrations, and policy. This is what you create during setup and what the [factory dashboard](/factories/factory-dashboard/) shows you.
...
* **Foreman name** - The handle your team @-mentions to reach the foreman from Slack and Linear, such as `@payments`. Setup copies it from the factory's name, so the two usually match, but what you mention is the foreman, which speaks for the factory. Change it under **Settings** > **Identity** in the factory dashboard, or with the [`alias`](/factories/factory-as-code/#alias) key in the definition.
* **Work item** - One request the factory is working, from intake to handoff. [Factory MCP](/factories/factory-mcp/) calls the same thing a task.
* **Factory MCP** - A hosted MCP server that connects a coding agent to your factories, so it can send work in or pull a work item down to finish locally. It is a way into a factory, not a factory itself. MCP clients conventionally key it `warp-factory`, a name you choose that has nothing to do with any factory's name or foreman handle.
...
flowchart LR
  ...
  Slack["Slack or Linear"] -->|"@Foreman name"| Foreman
  Coding["Your coding agent"] -->|"Factory MCP"| Foreman
```
