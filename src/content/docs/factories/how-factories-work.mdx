...
These are the stage names the [factory dashboard](/factories/factory-dashboard/) uses, where the **Activity** view is where you find, filter, and stop work items. Stages and agents are named separately, so the spec agent works the Planning stage, the implement agent works the Building stage, and the review agent works Reviewing. Each agent's run within a work item is an ordinary [cloud agent run](/platform/) you can watch and steer.
...
* **Approving the spec** - When work goes through the Planning stage, building waits until a person signs off on the plan.
