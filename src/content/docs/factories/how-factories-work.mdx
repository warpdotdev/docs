...
* **Intake** - A work item enters from a [connected integration](./connect-your-factory), an automation, a direct run, or the [Factory MCP](./factory-mcp), and keeps its source context.
...
Each completed work item leaves evidence behind: run and pull request activity, costs, Scorer evaluations, and benchmarks. Use it to spot repeated failures and compare model or harness configurations.

The factory can act on that evidence itself: [Self-improvement](./measure-and-improve#configure-and-review-self-improvement) groups the failures Scorers flag and files follow-up runs that can propose changes to application code or to the factory's own definition. Nothing is adopted without your review.
