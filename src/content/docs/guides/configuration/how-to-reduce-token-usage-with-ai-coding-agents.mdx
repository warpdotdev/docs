...
description: >-
  Reduce the tokens your coding agents consume in Warp using model choice and
  routing, focused context, conversation management, and Rules.
sidebar:
...
---
Every task your agents run consumes tokens, the units of text a model processes to read your request, your context, and its own output. The more tokens a task uses, the more it costs and the slower it can get, so trimming token usage keeps your agent workflows lean.
...
Token usage is non-deterministic, so treat these as habits that bring your usage down over time rather than exact, guaranteed savings. For a full breakdown of what drives usage, see [what affects usage](/support-and-community/plans-and-billing/credits/).

## Track your usage first

You can't reduce what you can't see. Before optimizing, build an intuition for which prompts, models, and workflows use the most tokens.

* **Per-turn breakdown** - Hover over the usage chip at the bottom of any agent response to see the tokens, tool calls, context window, and diffs that turn used.
...
Together, these habits lower your token usage over time: match the model to the task, keep conversations and context tight, and configure Rules and Codebase Context. Pair them with the right Agent Profile, and keep an eye on your usage as you go.
