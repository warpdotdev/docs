...
Each scheduled run starts a fresh cloud agent session and records its own task history, so you can audit what ran, when it ran, and what the agent did.
...
Name cloud runs consistently with `--name` so your team can filter and audit related runs later.
...
## Inspect and audit unattended runs
