...
<figcaption>Warp surfaces a notification when an agent needs approval, so you can jump directly to the tab that needs input.</figcaption>
</figure>
...
<figcaption>Save a tab config to recreate your multi-agent workspace with one click.</figcaption>
</figure>
