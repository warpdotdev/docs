...
:::caution
A purchase made by one member is billed to whichever card the team already has on file, which may be a card a different member added. If you don't want your card used this way, ask a team admin to replace or remove it in the billing portal, or email [billing@warp.dev](mailto:billing@warp.dev).
:::
...
You don't need a subscription to buy credits. On the Free plan you can purchase the same credit packs and spend them on any model, so a one-off top-up is an alternative to upgrading. Free plan packs cost 20% more than the paid-plan price.
...
On a shared team, that means one member's card can end up funding another member's purchase. If that isn't what you want, ask a team admin to replace or remove the saved card, lower the team-wide spend cap, or email [billing@warp.dev](mailto:billing@warp.dev) for help.
