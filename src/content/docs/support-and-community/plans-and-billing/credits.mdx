...
There's no exact formula for predicting usage. Experiment with different prompts and models and track how many credits they consume.
...
Compute credits are sometimes called **cloud agent credits**, since only cloud agent runs consume them. Both names refer to the same bucket.
...
* **{VARS.WARP_PLATFORM_API}** - Running agents through Warp's API
