...
* **{VARS.WARP_AUTOMATION_PLATFORM} API** - Running agents through Warp's {VARS.WARP_AUTOMATION_PLATFORM} API
...
* **Third-party agent CLIs run directly** don't use platform credits when you run `claude`, `codex`, or another agent CLI outside of {VARS.WARP_AUTOMATION_PLATFORM}.
