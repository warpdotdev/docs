...
Platform credits cover Warp's platform infrastructure for coordinating, observing, and integrating agent runs. They apply to every cloud agent run, plus local agent runs on Business and Enterprise plans that use customer-supplied inference such as BYOK, a custom inference endpoint, or BYOLLM via AWS Bedrock (with Azure Foundry and Google Vertex coming soon).
...
Customer-supplied inference covers [Bring Your Own API Key (BYOK)](/agent-platform/inference/bring-your-own-api-key/), [custom inference endpoints](/agent-platform/inference/custom-inference-endpoint/), and BYOLLM via AWS Bedrock (with Azure Foundry and Google Vertex coming soon).

:::caution
BYOK and customer-supplied inference (custom inference endpoints, plus BYOLLM via AWS Bedrock — with Azure Foundry and Google Vertex coming soon) are available to individual users and organizations with 10 or fewer employees on any plan. Organizations with more than 10 employees need a Warp Business or Enterprise plan to use BYOK or customer-supplied inference. See Warp's [Terms of Service](https://www.warp.dev/terms-of-service) for details.
:::
