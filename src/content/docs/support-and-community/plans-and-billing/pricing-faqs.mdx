...
For details on Warp's legacy plans (Pro, Turbo, Lightspeed), see [Overages (Legacy)](/support-and-community/plans-and-billing/overages-legacy/). For the most up-to-date feature and usage details — including current per-plan seat limits and monthly credit allowances — visit [warp.dev/pricing](https://www.warp.dev/pricing).
...
* **Grandfathered pooled credits** — Teams that purchased Reload credits (formerly Add-on Credits) before May 21, 2026 keep their existing pooled balance until it's exhausted. Pooled credits are spent down first across the team; all new Reload credit purchases after May 21, 2026 are user-scoped.
...
The following questions cover the May 21, 2026 pricing updates: seat limits per plan, Reload credits attribution, Max plan credit allowance changes, BYOK on Free, the new Custom Inference Endpoint feature, and ZDR / data-controls clarifications.
...
Before May 21, 2026, Reload credits (previously called Add-on Credits) on multi-seat teams were **pooled** — every team member drew from a single shared balance. As of May 21, 2026, **Reload credits are user-scoped**: each user has their own balance, and a single heavy user can no longer drain the whole team's purchased credits.
...
Yes. The Max plan's monthly credit allowance is decreasing as of May 21, 2026. For the current allowance, see [warp.dev/pricing](https://www.warp.dev/pricing).
...
On **July 1, 2026**, Warp begins charging self-serve customers platform credits for:

* Every cloud agent run on Build, Max, or Business.
* Local agent runs on Business that use BYOK or a custom inference endpoint.

This date lines up with the rest of the May 2026 self-serve pricing transition (seat limits, Max credit allowance change, user-scoped Reload credits, and related grandfathering changes).
