...
1. Go to the [Warp login page](https://app.warp.dev/login).
2. Log in with the original method that you used to create your Warp account (email, Google, GitHub).
3. Once logged in, go to the [SSO linking page](https://app.warp.dev/link_sso).
4. This should link your login to SSO. You can now proceed to log in with "Continue with SSO".
...
Please note that creating multiple accounts or using throwaway emails is against our [Terms of Service](https://www.warp.dev/legal/terms-of-service) and increases the chance of triggering this system significantly.
...
## How to get an auth token to log in
...
\
On Linux and WSL you should install and set your default `$BROWSER` to `brave-browser` to work around any copy-paste issues. Please see the workaround guide below.
:::
