...
When you start an [Agent conversation](/agents/local-agents/interacting-with-agents/), the Agent receives a list of all available skills with their names and descriptions.
...
### Identify skill sources in conversation data

The [Conversation API](/api) returns skill loads as `read_skill` actions. The action's `input` identifies the skill source:

| Input field | Skill source |
| --- | --- |
| `bundled_skill_id` | A Warp-provided skill bundled with the product. |
| `path` | A file-based skill discovered from a project, your home directory, an extra skill directory, or another configured source. |

To measure usage of skills your team owns, exclude `read_skill` actions whose `input` contains `bundled_skill_id`. From the remaining actions, count only `path` values under directories your team manages. A path identifies a file-based skill, but the path's location determines its owner.

The conversation or run payload is the source of truth for which skills were available and invoked. The bundled set can vary by Warp release, release channel, feature availability, required files, and connected integrations.

#### Bundled skill ID catalog

The following IDs cover the stable, user-facing skills bundled directly with Warp. This catalog helps group usage across conversations; it isn't an availability manifest.
...
For example, `oz-platform`, `factory-files`, and `factory-mcp` are Warp-provided skills. `factory-mcp` appears only where Factory MCP is available, `tui-migrate-setup` is specific to the Warp Agent CLI, and some other skills depend on a feature, bundled resource, or integration. Connected integrations can also add bundled IDs that aren't listed here.
...
Use the [`/open-skill`](/agents/capabilities/slash-commands/) slash command to modify existing skills:
...
These same skills also appear as suggested agents in the [{VARS.WEB_APP}](/platform/oz-web-app/), where you can run them directly in the cloud.

## Suggested skills from Agent Memory

Promoting recurring patterns from [Agent Memory](/agents/agent-memory/) into reviewable skill drafts is in design as part of the research preview. See the Agent Memory page for a current status.
...
Skills can be used with both local and [cloud agents](/platform/) to create reusable, automated workflows. When running an agent via the CLI, web app, or API, you can specify a skill to provide the base instructions for the agent.

For a complete guide to running skill-based agents—including CLI usage, the {VARS.WEB_APP}, scheduling, skill discovery, and API integration—see [Skills as Agents](/platform/skills-as-agents/).

## Related features

* [**Rules**](/agents/capabilities/rules/) - Set persistent guidelines and constraints for Agent behavior
* [**MCP Servers**](/agents/capabilities/mcp/) - Expose external data sources and tools to Agents
* [**Cloud Agents**](/platform/) - Run Agents in the cloud on schedules or triggers
* [**Agent Profiles**](/agents/capabilities/agent-profiles-permissions/) - Control Agent permissions and autonomy
...
* **[Scheduled Agents quickstart](/platform/triggers/scheduled-agents-quickstart/)** - Run a skill on a recurring cron schedule for tasks like weekly dependency checks or daily code cleanup.
* **[Integrations quickstart](/platform/integrations/quickstart/)** - Trigger skills from Slack or Linear so your team can invoke agent workflows from the tools they already use.
