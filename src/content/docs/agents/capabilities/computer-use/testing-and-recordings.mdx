...
- **Synthetic cursor** - The sandbox capture contains no hardware cursor, so the burn-in draws its own arrow glyph — a white pointer with a dark outline that stays legible on any background — and moves it along the recorded pointer coordinates. The cursor eases into each interaction over roughly half a second and arrives exactly as the action fires, so the video reads as a smooth demo rather than a series of jumps.
