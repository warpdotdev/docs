...
1. Run `oz federate issue-token` to get the OIDC JWT.
2. Call the [Security Token Service `token` endpoint](https://cloud.google.com/iam/docs/factories/developer-tools/sts/rest/v1/TopLevel/token) to exchange the JWT for a federated access token.
