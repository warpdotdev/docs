...
* **Grok Build** — xAI's CLI coding agent (invoked with `grok`)
...
**Goose**, **Antigravity** (`agy`), **Hermes**, **Mistral Vibe**, and **Grok Build** (`grok`) are also recognized. They get the same enhanced features as the agents above — rich input editor, code review comments, attach code as context, vertical tabs and metadata, Tab Configs, and Remote Control — but don't support agent notifications yet.

:::note
Agent notifications require a one-time setup. Claude Code and OpenCode use a Warp notification plugin. Codex uses a native config change. See the individual agent pages for setup instructions. Amp, Auggie, Copilot CLI, Cursor, Gemini CLI, Droid, Pi, Goose, Antigravity, Hermes, Mistral Vibe, and Grok Build don't support notifications yet.
:::
