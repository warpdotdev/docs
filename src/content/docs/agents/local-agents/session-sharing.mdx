...
Stopping a share immediately ends live access for every participant — no one can continue watching or interacting once you stop it. The underlying shared data then goes away on its own at the one-week expiry along with the rest of your shared session data. There's currently no separate control to delete a single shared session's data before that expiry.

:::note
Shared [Blocks](/terminal/blocks/block-basics/) work differently. Unsharing a Block permanently deletes it right away. Session shares rely on the one-week expiry instead of an immediate-delete action.
:::

To remove all of your shared session data immediately rather than waiting for it to expire, delete your Warp account and data. See [Delete your account and data](/support-and-community/privacy-and-security/privacy/#delete-your-account-and-data).
...
* **Anyone with the link** - Only if you explicitly turn this on for the session
