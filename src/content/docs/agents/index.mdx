...
This page covers where to run the agent, the third-party CLI agents Warp supports, and [{VARS.WARP_AUTOMATION_PLATFORM}](/platform/overview/), the platform that runs and coordinates agents at scale.
...
[**{VARS.WARP_AUTOMATION_PLATFORM}**](/platform/overview/) is Warp's programmable platform for running and coordinating agents at scale. It provides the environments, triggers, integrations, orchestration, and observability that cloud agents run on, along with a CLI, API, and SDK for driving agents programmatically.
