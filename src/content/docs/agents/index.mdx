...
This page covers where to run the agent, the third-party CLI agents Warp supports, and [{VARS.WARP_AUTOMATION_PLATFORM}](/platform/overview/), which runs and coordinates agents at scale.
...
* [**In the cloud, as a cloud agent**](/platform/) - Run the agent in the background from a trigger, a schedule, or an integration, without keeping a session open. Cloud agents run on [{VARS.WARP_AUTOMATION_PLATFORM}](/platform/overview/), Warp's platform for environments, triggers, orchestration, and observability.
