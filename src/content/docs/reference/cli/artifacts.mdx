...
Artifacts are files that an agent produces during a run and uploads to {VARS.WARP_AUTOMATION_PLATFORM} — screenshots, generated reports, build outputs, logs, or any other file the agent saves alongside its conversation. Use `oz artifact` to inspect those files from outside the run and pull them down to your machine.
...
Artifacts are referenced by an artifact UID. You can find UIDs in the agent's run detail view, in the JSON returned by [`oz run get`](/reference/cli/), or in the response from the [{VARS.WARP_AUTOMATION_PLATFORM} API](/reference/api-and-sdk/).
