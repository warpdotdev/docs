...
3. **Configure** the {VARS.WARP_AUTOMATION_PLATFORM} app with an integration.
...
For automated workflows that use an [agent API key](/reference/cli/api-keys/) (CI/CD pipelines, scheduled agents, SDK-triggered runs), you can configure team GitHub authorization so the agent authenticates with the {VARS.WARP_AUTOMATION_PLATFORM} by Warp GitHub App instead of an individual's personal token.
...
2. Opens a browser flow to install the {VARS.WARP_AUTOMATION_PLATFORM} app into your Slack workspace or Linear workspace.
3. Generates an **integration ID** you can later list or delete.

:::note
The [GitHub integration](/platform/integrations/github/), which starts agents from `@oz-agent` mentions on issues and pull requests, isn't created with `oz integration create`. It's set up by installing the {VARS.WARP_AUTOMATION_PLATFORM} by Warp GitHub App and enabling the GitHub organization in the Admin Panel.
:::
