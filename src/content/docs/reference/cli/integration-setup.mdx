...
:::note
**Using API keys:** If you're triggering agents via API key rather than through a Slack or Linear integration, use a [personal API key](/reference/cli/api-keys#personal-vs-team-api-keys) for workflows where the agent needs to write to a repository. Personal keys authenticate as you, so the agent runs with your GitHub permissions.
:::
