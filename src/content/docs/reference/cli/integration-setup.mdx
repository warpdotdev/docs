...
:::note
**Using API keys:** Agent API keys with [team GitHub authorization](/agent-platform/cloud-agents/team-access-billing-and-identity/#team-github-authorization) configured use the GitHub App token instead of an individual's personal token. Use `oz login` for local or remote user-authenticated CLI runs that should use your GitHub permissions.
:::
