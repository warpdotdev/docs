...
:::caution
The {VARS.WARP_AGENT_CLI} (the `oz` binary, which previously shipped as `warp-cli`) is being deprecated in favor of the {VARS.WARP_CLI} (the `warp` binary). `oz` commands remain supported through the end of September 2026, but the successor `warp` CLI commands for these workflows haven't been defined yet. See the [Warp Agent CLI docs](/agents/cli/) for what's available today.
:::
