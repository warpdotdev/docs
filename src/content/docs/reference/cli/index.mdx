...
The Oz CLI is the command-line tool that lets you run [Cloud Agents](/platform/) from anywhere, including terminals, scripts, automated systems, or services.
...
<Tabs>
  <TabItem label="macOS">
    On macOS, install and update the standalone CLI with [Homebrew](https://brew.sh/), using the [`warpdotdev/warp` tap](https://github.com/warpdotdev/homebrew-warp):
  ...
  <TabItem label="Linux">
    On Linux, install and update the standalone CLI through your distribution's package manager. Warp supports `apt`, `yum`, and `pacman`.
...
The CLI prints out a URL that you can open in any browser to log in to Warp.
