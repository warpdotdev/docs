...
description: >-
  Create and monitor your first cloud agent run via the {{WARP_AUTOMATION_PLATFORM}} API or SDK in ~5
  minutes.
sidebar:
  label: "Quickstart"
---
...
The {VARS.WARP_AUTOMATION_PLATFORM} API lets you run and manage cloud agents from anywhere — CI/CD pipelines, backend services, scripts, or custom tooling — without the Warp desktop app. This quickstart walks you through creating your first run and checking its status.
...
<VideoEmbed url="https://youtu.be/N6qMe641K34" title={`${VARS.WARP_AUTOMATION_PLATFORM} API and SDK quickstart video`} />
...
* **An {VARS.WARP_AUTOMATION_PLATFORM} cloud environment** - Agents run inside a configured environment that includes repos and other dependencies. If you don't have an environment yet, follow the [Cloud Agents Quickstart](/platform/quickstart/) first.
...
* **Read the full API reference** - [{VARS.WARP_AUTOMATION_PLATFORM} API](/reference/api-and-sdk/) documents all endpoint parameters, query filters, and response schemas.
