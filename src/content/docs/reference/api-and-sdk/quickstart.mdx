...
* **A Warp API key** - Create one in the [{VARS.WEB_APP}]({VARS.WEB_APP_URL}/settings) and copy the raw value. Use a personal key if you want runs attributed to you, or an agent key to attribute runs to a [cloud agent](/platform/agents/). See [API Keys](/reference/cli/api-keys/) for the full flow.
...
Replace `<ENV_ID>` with your environment ID. Find it with `oz environment list` on the {VARS.WARP_AGENT_CLI} or in the [{VARS.WEB_APP}]({VARS.WEB_APP_URL}).
...
You can also view and manage all runs in the [{VARS.DASHBOARD}]({VARS.WEB_APP_URL}/runs).
