...
The Conversation API returns skill loads as `read_skill` actions. An action with `input.bundled_skill_id` loaded a Warp-provided skill. An action with `input.path` loaded a file-based skill.
